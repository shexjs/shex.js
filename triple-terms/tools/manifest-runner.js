...
function loadEntry (entry, dir, options = {}) {
  ...
  for (const text of [].concat(textOf(entry, "data", dir) === undefined ? [] : textOf(entry, "data", dir))) {
    ...
    const parser = new N3.Parser({baseIRI: base, format: "application/trig", blankNodePrefix: ""}); // TriG ⊇ Turtle (doc/datasets.md), RDF 1.2 triple terms included (doc/triple-terms.md)
