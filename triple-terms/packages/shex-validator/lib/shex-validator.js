...
class ShapeExprValidationContext {
    ...
    followTripleConstraint(graphView = this.graphView) {
     ...
     */
    forkTripleConstraint(graphView = this.graphView) {
        return new ShapeExprValidationContext(this, this.label, this.depth + 1, this.tracker, Object.assign({}, this.seen), this.matchTarget, null, false, graphView);
    }
...
class ShExValidator {
    ...
    *resumeShapeExpr(focus, shapeExpr, ctx) {
        ...
        switch (shapeExpr.type) {
            ...
            case "TripleTermConstraint": { // doc/triple-terms.md
                const ttc = shapeExpr;
                if (focus.termType !== "Quad")
                    return { type: "Failure", node: (0, term_1.rdfJsTerm2Ld)(focus), shape: ctx.label, errors: [
                            { type: "NodeKindMismatch", expected: "tripleterm", actual: focus.termType.toLowerCase() }
                        ] };
                const tt = focus;
                const ttErrors = [];
                const parts = {};
                ...
                for (const [member, expr] of [["subject", ttc.subject], ["object", ttc.object]]) {
                    if (expr === undefined)
                        continue;
                ...
                return ttErrors.length > 0
                    ? { type: "Failure", node: (0, term_1.rdfJsTerm2Ld)(focus), shape: ctx.label, errors: ttErrors }
                    : Object.assign({ type: "TripleTermTest", node: (0, term_1.rdfJsTerm2Ld)(focus) }, parts);
            }
            default:
    ...
    *matchByPredicate(constraintList, neighborhood, ctx) {
        ...
        for (const constraint of constraintList) {
            ...
            const arcView = typeof constraint.graph === "string"
                ? { termType: "NamedNode", value: constraint.graph }
                : ctx.graphView;
    ...
    *triplesMatchingShapeExpr(triples, constraint, ctx) {
        ...
        if (this.canFork() && triples.length > 1 && constraint.valueExpr !== undefined
            && constraint.graph === undefined // datasets: graph switches take the sequential path
            ...
            if (constraint.valueExpr === undefined)
                ...
                if ("error" in gv) {
                    misses.push(new TriplesMatchingMiss(triple, {
                        type: "Failure", node: (0, term_1.rdfJsTerm2Ld)(value), shape: "GRAPH", errors: [gv.error],
                    }));
                    continue;
                }
    ...
    validateNodeConstraint(focus, nc, ctx) {
        ...
        if (nc.nodeKind !== undefined) {
            ...
            if (focus.termType === "Quad") { // doc/triple-terms.md: TRIPLE matches
                ...
                if (nc.nodeKind !== "tripleterm") {
                    validationError({ type: "NodeKindMismatch", expected: nc.nodeKind, actual: "tripleterm" }, `triple term found when ${nc.nodeKind} expected`);
                }
            }
            else if (nc.nodeKind === "tripleterm") {
                validationError({ type: "NodeKindMismatch", expected: nc.nodeKind, actual: focus.termType.toLowerCase() }, `${focus.termType} found when triple term expected`);
            }
...
function childGraphView(constraint, value, ctx) {
    const graph = constraint.graph;
    if (graph === undefined)
    ...
    if (graph.type === "GraphTerm")
        return value.termType === "NamedNode" || value.termType === "BlankNode"
            ...
            : { error: { type: "GraphNameViolation", graphSpec: "TERM", node: (0, term_1.rdfJsTerm2Ld)(value) } };
    ...
    return value.termType === "NamedNode"
        ...
        : { error: { type: "GraphNameViolation", graphSpec: "FRAGMENT", node: (0, term_1.rdfJsTerm2Ld)(value) } };
}
/** the neighborhood a shape sees under a view: its arcs, plus those in any
 * graph a GRAPH <g> constraint of the shape names */
function applyGraphView(fromDB, tripleConstraints, ctx) {
    if (ctx.graphView === null || ctx.subGraph) // no view, or a partition already chosen
        return fromDB;
    const view = ctx.graphView;
    const fixed = new Set(tripleConstraints.map(tc => tc.graph)
        .filter(g => typeof g === "string"));
    const keep = (q) => sameGraphTerm(q.graph, view) || fixed.has(q.graph.value);
