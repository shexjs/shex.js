...
/** an entry of the output ShapeMap as parseMaterializationShapeMap resolves
 * it: the node to invent and the shape to build it against, both terms */
type ShapeMapPair = {node: any, shape: any};

/** index of the first "node@shape" separator in an output ShapeMap ('@'
 * outside <>s), or -1 if there is none yet */
function outputMapAtIndex (text: string): number {
  let depth = 0;
...
function quadKey (q: any): string {
  const term = (t: any) => t.termType + "\u0000" + t.value +
 ...
 */
function orderedForNesting (quads: any[]): any[] {
  const seen = new Set();
...
class DirectShExMaterializer {
  ...
  frames: any[];
  frameOrigins: any[];
  constructor (schema: any, shapeMap: ShapeMapPair[], resultBindings: any, staticVars: any, mapModule: any) {
    this.schema = schema;
  ...
  async invoke (fixedMap: any, validationTracker: any, time: any, _done: any, _currentAction: any) {
...
class RemoteShExMaterializer {
  ...
  schemaTcs: any[];
  created: Promise<any>;
  // error() reaches these two and nothing assigns them: the constructor is
  // handed a resultsWidget it does not keep, and there is no renderEntry
  resultsWidget: any;
  renderEntry: (entry: any) => void;
  constructor (schema: any, shapeMap: ShapeMapPair[], resultBindings: any, staticVars: any,
               resultsWidget: any, onCancel: () => void, workerUrl: string, mapModule: any) {
  ...
  async invoke (fixedMap: any, validationTracker: any, time: any, done: any, currentAction: any) {
...
const ShExMapVerbs = {
  ...
  getMaterializer (schema: any, shapeMap: ShapeMapPair[], resultBindings: any, staticVars: any) {
    return this.remote
  ...
  debugStep (command: "continue" | "stepInto" | "stepOver" | "stepOut") {
  ...
  bindingStateText (thread: any) {
    ...
    const prefixes: {[prefix: string]: string} = this.Caches.outputSchema.parsed._prefixes || {};
    const pname = (iri: string) => {
  ...
  stepLabel (event: any, thread: any) {
   ...
   */
  annotateBindingState (thread: any) {
  ...
  previewThread (thread: any, complete: boolean, label: string) {
    const session = this.debugSession;
    this.clearResults();
   ...
   */
  writeNestedTurtle (quads: any[], onDone: (error: any, result: string) => void) {
  ...
  renderAcceptAlternatives (materializer: any, outputShapeMap: ShapeMapPair[]) {
   ...
   */
  fixMaterializationShapeMapEntry (node: string, shape: string): ShapeMapPair {
    return {
   ...
   */
  parseMaterializationShapeMap (text: string): ShapeMapPair[] {
    const pairs = [];
  ...
  reportMaterializationError (materializationError: any, currentAction: string) {
  ...
  anchorMaterializationFailures (e: any, lastReport?: any) {
    const pane = this.editorSupport && this.editorSupport.panes.outputSchema;
    if (!pane)
      return;
    try {
  ...
  renderMaterializedGraph (generatedGraph: any, outputShapeMap: ShapeMapPair[], provenance: any[]) {
    try {
  ...
  reportMaterialization (provenance: any[], focus?: any) {
  ...
  bindingRangesAt (text: string, path: any[]) {
   ...
   */
  bindingRanges (text: string, variable: string, frame: number | null) {
    const origins = this.bindingOrigins;
    if (!origins)
  ...
  variableRanges (text: string, variable: string, frame: number | null) {
  ...
  setMaterializationHovers (rendered: {pane: any, text: string, pairs: any[]}[], focus: any) {
    ...
    const show = (group: any[], hoveredPane: any, pinning?: boolean) => {
  ...
  addResult (error: any, result: string) {
  ...
  bindingsToTable () {
    ...
    const shown = $("#bindings1").find(".shexjs-editor-pane, textarea")
          .filter((i: any, elt: any) => elt.style.display !== "none").first()
...
ShExPlugins.register({
  ...
  register (validator: any, api: any) {
  ...
  results: (base: any) => class extends base {
  ...
  /** a validation replaces the bindings a materialization was made from, so
   * its results are no longer about anything: the tab goes */
  onStartingValidation (app: any) { app.hideMaterialization(); },
