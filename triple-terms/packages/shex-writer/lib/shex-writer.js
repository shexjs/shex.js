...
const RDF_PREFIX = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#', RDF_TYPE = RDF_PREFIX + 'type';
...
class ShExWriter {
    constructor(outputStream, options) {
        ...
        if (!outputStream) {
            let output = '';
            this._outputStream = {
                write: function (chunk, _encoding, done) { output += chunk; done && done(); },
            ...
            function _writeExpression(expr, indent, parentPrecedence) {
                ...
                if (typeof expr === "string") {
                    ...
                    if (expr.type === "TripleConstraint") {
                        ...
                        const graph = expr.graph; // datasets strawman (doc/datasets.md)
                        if (graph !== undefined)
                            pieces.push("GRAPH ", typeof graph === "string" ? _ShExWriter._encodeShapeName(graph, false)
    ...
    _encodeIriOrBlankNode(iri, trailingSpace) {
    ...
    _encodeLiteral(value, type, language) {
        // Escape special characters
    ...
    _encodeShapeName(subject, trailingSpace) {
        if (subject[0] === '"')
    ...
    addShape(shape, name, done) {
        this._write(this._encodeShapeName(name, false) +
            " " +
            this._writeShapeExpr(shape, done, true, 0).join(""), done);
    }
