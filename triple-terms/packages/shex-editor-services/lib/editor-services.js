    ...
    (function walk(expr) {
        ...
        if (expr.type === "TripleConstraint") {
            ...
            if (range) {
                ...
                if (min === null || inner.from < min)
                    min = inner.from;
                if (max === null || inner.to > max)
                    max = inner.to;
            }
        }
        else if (expr.expressions)
...
function innerRange(range, delim, text) {
...
function mapValidationErrors(valResult, shexcParsed, turtleParsed, opts = {}) {
    ...
    function emit(status, leaf, err, ctx) {
        const ca = leaf.predicate && ctx.shape
            ...
            : null;
        let schemaRange = (leaf.schemaObj && shexcParsed.locate.expr(leaf.schemaObj)) ||
            (ca && ca.parts[0]) ||
        ...
        if (turtleParsed && turtleParsed.quads) {
            ...
            if (leaf.reifiesOuter && anchors.object) {
                ...
                anchors.objectParts = [{ from: s.from, to: s.from + 2 }, { from: s.to - 2, to: s.to }];
