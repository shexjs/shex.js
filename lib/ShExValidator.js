...
function ShExValidator(schema, options) {
  ...
  this._validatePoint = function (graph, node, outerExpression, constraintList, constraintToTripleMapping, tripleToConstraintMapping, neighborhood) {
    var _this = this;
    var newThreads = [];
...
// !! copied
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      var fromValidatePoint = this._validatePoint(db, point, shape.expression, constraintList, _constraintToTriples(), tripleToConstraintMapping, neighborhood);
      // var nfaTrace = [];
      // var fromNFA = nfa.match(db, point, nfaTrace, constraintList, _constraintToTriples(), tripleToConstraintMapping);
      // if ("errors" in fromNFA !== "errors" in fromValidatePoint)
      //    { throw Error(JSON.stringify(fromValidatePoint) + " vs " + JSON.stringify(fromNFA)); }
      if (!("errors" in fromValidatePoint)) {
        function _dive (solns) {
          if (solns.type === "someOfSolutions" ||
            ...
            solns.solutions = solns.solutions.map(x => {
              ...
              var subErrors = _ShExValidator._errorsMatchingValueExpr(expr.inverse ? t.subject : t.object, expr.valueExpr, expr.inverse, function (focus, shape) {
                var sub = _ShExValidator.validate(db, focus, shape, depth+1, seen)
                if ("errors" in sub) {
                  // console.dir(sub);
                  return [{
              ...
              if (subErrors.length === 0)
                if (!_ShExValidator.semActHandler.dispatchAll(expr.semActs || [], t))
      ...
      if ("errors" in fromValidatePoint) {
        partitionErrors.push({
          errors: fromValidatePoint.errors
        });
      ...
      if (!this.semActHandler.dispatchAll(shape.semActs || [], fromValidatePoint))
        continue; // some semAct aborted
//      if ("semActs" in shape) { fromNFA.semActs = shape.semActs; } !!! needs test case
      ...
      ret = { type: "test", node: point, shape: shapeLabel, solution: fromValidatePoint }; // expression: expr, triples: usedTriples
