        ...
        if (N3Util.isLiteral(value) && valueExpr.values.reduce((ret, v) => {
          ...
          if (!(valueExpr.values.some(function (valueConstraint) {
            if (typeof valueConstraint === "object" && !("value" in valueConstraint)) { // isTerm me -- strike "value" in
              ...
              function startsWith (val, ref) { // !!! language or datatype?
                var v = val.startsWith("\"") ? val.substr(1, val.length - 2) : val;
                return v.startsWith(typeof ref === "object" && "value" in ref ? ref.value : ref);
              }
              function equals (val, ref) {
                var v = val.startsWith("\"") ? val.substr(1, val.length - 2) : val;
                return v === (typeof ref === "object" && "value" in ref ? ref.value : ref);
              }
