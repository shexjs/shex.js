...
function ShExValidator(schema, options) {
  ...
  this._triplesMatchingValueExpr = function (triples, valueExpr, inverse) {
    ...
    triples.forEach(function (triple) {
      ...
      var errors = _ShExValidator._errorsMatchingValueExpr(value, valueExpr, inverse);
      if (errors.length === 0) {
        hits.push(triple);
  ...
  this._errorsMatchingValueExpr = function (value, valueExpr, inverse) {
    var _ShExValidator = this;
    if (valueExpr.type === "ValueClass") {
      return this._errorsMatchingValueClass (value, valueExpr, inverse);
    } else if (valueExpr.type === "ValueRef") {
      return this._errorsMatchingValueExpr (value, schema.valueExprDefns[valueExpr.valueExprRef].valueExpr, inverse);
    } else if (valueExpr.type === "ValueOr") {
      var ret = [];
      for (var i = 0; i < valueExpr.valueExprs.length; ++i) {
        var nested = _ShExValidator._errorsMatchingValueExpr(value, valueExpr.valueExprs[i], inverse);
        if (nested.length === 0)
          return nested;
      ...
      return valueExpr.valueExprs.reduce(function (ret, nested, iter) {
        return ret.concat(_ShExValidator._errorsMatchingValueExpr (value, nested, inverse));
      }, []);
    } else {
   ...
   */
  this._errorsMatchingValueClass = function (value, valueExpr, inverse) {
      var errors = [];
