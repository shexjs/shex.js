...
function ShExValidator(schema, options) {
  ...
  this.compileNFA = function (expression) {
    ...
    function rbenx_match (graph, node, trace, constraintList, constraintToTripleMapping, tripleToConstraintMapping) {
      ...
      addstate(clist, this.start, {repeats:{}, avail:[], matched:[], stack:[]}); // a thread which repeats no states
      while (clist.length) {
        nlist = [];
        if (trace)
          trace.push({threads:[]});
  ...
  this.matchedToResult = function (matched, neighborhood, recurse, semActHandler) {
    ...
    var ret = matched.reduce((out, m) => {
      ...
      ptr.solutions = m.triples.map(tno => {
        ...
        errors = errors.concat(_ShExValidator._errorsMatchingValueExpr(ptr.inverse ? triple.subject : triple.object, ptr.valueExpr, function (focus, shape) {
