...
function ShExValidator(schema, options) {
    ...
    if (shapeExpr.type === "Shape") {
      ...
      ret = {
        ...
        shapeExpr.shapeExprs.map(shape => {
          return this._validateShape(db, point, regexModule.compile(schema, shape),
  ...
  this._validateShape = function (db, point, regexEngine, shape, shapeLabel, depth, seen) {
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      function _testExpr (term, valueExpr, recurse) {
        return _ShExValidator._errorsMatchingValueExpr(term, valueExpr, recurse)
      }
      var results = regexEngine.match(db, point, constraintList, _constraintToTriples(), tripleToConstraintMapping, neighborhood, _recurse, this.semActHandler, _testExpr, null);
  ...
  this._triplesMatchingValueExpr = function (triples, valueExpr, inverse, recurse) {
    ...
    triples.forEach(function (triple) {
      ...
      var errors = _ShExValidator._errorsMatchingValueExpr(value, valueExpr, recurse);
      if (errors.length === 0) {
        hits.push(triple);
  ...
  this._errorsMatchingValueExpr = function (value, valueExpr, recurse) {
    var _ShExValidator = this;
    if (valueExpr.type === "ValueClass") {
      return this._errorsMatchingValueClass (value, valueExpr, recurse);
    } else if (valueExpr.type === "ValueRef") {
      return this._errorsMatchingValueExpr (value, schema.valueExprDefns[valueExpr.valueExprRef].valueExpr, recurse);
    } else if (valueExpr.type === "ValueOr") {
      var ret = [];
      for (var i = 0; i < valueExpr.valueExprs.length; ++i) {
        var nested = _ShExValidator._errorsMatchingValueExpr(value, valueExpr.valueExprs[i], recurse);
        if (nested.length === 0)
          return nested;
      ...
      return valueExpr.valueExprs.reduce(function (ret, nested, iter) {
        return ret.concat(_ShExValidator._errorsMatchingValueExpr (value, nested, recurse));
      }, []);
    } else {
