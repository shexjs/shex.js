...
function ShExValidator(schema, options) {
  ...
  this.validate = function (db, point, shapeLabel, depth) {
    ...
    var constraintList = this._shapeCompilationByLabel[shapeLabel].tripleConstraints;
    if (depth === undefined)
      depth = 0;
   ...
   function _log () {
    ...
    function indexNeighborhood (triples) {
      var neighborhood = {}
      neighborhood.triples = triples;

      /* byPredicate - mapping from predicate to triples containing that
       * predicate.
       */
      neighborhood.byPredicate = 
        neighborhood.triples.reduce(function (ret, t) {
          var p = t.predicate;
          if (!(p in ret))
            ret[p] = [];
          ret[p].push(t);
      ...
      */
      neighborhood.candidates = _seq(neighborhood.triples.length).map(function () {
        return [];
      });
      return neighborhood;
    }
...
//    @@TODO - move _tripleConstraintsByLabel[] to _tripleConstraintsByLabel[].forard,reverse
    constraintList.forEach(function (tp, tpIndex) {
      var searchSubject = tp.inverse ? null : point;
      var searchObject = tp.inverse ? point : null;
      var matchPredicate = outgoing.byPredicate[tp.predicate] || [];
      var matchConstraints = _triplesMatchingPattern(matchPredicate, tp);
      matchConstraints.forEach(function (t) {
        outgoing.candidates[outgoing.triples.indexOf(t)].push(tpIndex);
      });
      // var x = _triplesMatchingDependentShape(db, matchConstraints, tp, depth);
    });
    // console.log(outgoing.candidates);
    // outgoing.candidates.forEach(function (candidate) { // limit to OPEN predicates
    //   candidate.push(ε);
    // });

    var ret = null;
    var r = new RegExp("^"+_ShExValidator._shapeCompilationByLabel[shapeLabel].regexp+"$"); // ((0 )*(1 )+|(2 )(3 ))
    var xp = crossProduct(outgoing.candidates);
    while (outgoing.candidates.length !== 0 && xp.next() && ret === null) xp.
      do(function () {
        ...
        var constraintMatchCount = _seq(outgoing.triples.length).map(function () { return 0; }); /* Map[Triple, Int] */
        for (var i = 0; i < arguments.length; ++i) {
          var tpNumber = arguments[i];
          tripleToConstraintMapping.push(tpNumber);
          if (tpNumber !== undefined) {
            if (tpNumber !== ε) {
              usedTriples.push(outgoing.triples[i]);
            }
        ...
        var byConstraint =
          _ShExValidator._shapeCompilationByLabel[shapeLabel].optimize.hasRepeatedGroups ?
        ...
        function _toVal (expr, constraintToTriples) { // consumes constraintToTriples
          if (expr.type === 'tripleConstraint') {
            ...
            while (constraintToTriples[constraintNo].length && (take === "*" || take--)) {
              var tripleNo = constraintToTriples[constraintNo].shift();
              var triple = outgoing.triples[tripleNo];
              ret.solutions.push({
                type: "testedTriple",
...
debugger;
    ...
    if (VERBOSE) { // remove N3jsTripleToString
      outgoing.triples.forEach(function (t) {
        delete t.toString;
      });
    }
    return ret;

    var alts = []; // cartesianProductOf(outgoing.candidates)
