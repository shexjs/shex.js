...
function ShExValidator_constructor(schema, options) {
  ...
  this._validateShape = function (db, point, regexEngine, shape, shapeLabel, tracker, seen) {
    ...
    var tripleList = constraintList.reduce(function (ret, constraint, cNo) {
      ...
      // validation closures to avaoid passing lots of parameters.
      function _validateByLabel (focus, shapeLabel) {
        var sub = _ShExValidator.validate(db, focus, shapeLabel, tracker, seen);
        return sub;
      }
      function _validateBySExpr (focus, shapeExpr) {
  ...
  this._triplesMatchingShapeExpr = function (triples, constraint, validateByLabel, validateBySExpr) {
    var _ShExValidator = this;
    var misses = [];
    var hits = [];
    triples.forEach(function (triple) {
      ...
      var errors = constraint.valueExpr === undefined ?
          undefined :
          (sub = _ShExValidator._errorsMatchingShapeExpr(value, constraint.valueExpr, validateByLabel, validateBySExpr)).errors;
  ...
  this._errorsMatchingShapeExpr = function (value, valueExpr, validateByLabel, validateBySExpr) {
    var _ShExValidator = this;
    if (valueExpr.type === "NodeConstraint") {
      ...
      return validateByLabel(value, valueExpr.reference);
    } else if (valueExpr.type === "ShapeOr") {
      var errors = [];
      for (var i = 0; i < valueExpr.shapeExprs.length; ++i) {
        var nested = valueExpr.shapeExprs[i];
        var sub = _ShExValidator._errorsMatchingShapeExpr(value, nested, validateByLabel, validateBySExpr, true);
        if ("errors" in sub)
          errors.push(sub);
        else
          return { type: "ShapeOrResults", solution: sub };
      ...
      var sub = _ShExValidator._errorsMatchingShapeExpr(value, valueExpr.shapeExpr, validateByLabel, validateBySExpr, true);
      // return sub.errors && sub.errors.length ? {} : {
