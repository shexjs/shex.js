...
function ShExValidator(schema, options) {
  ...
  function compileShapeToAST (expression) {
      ...
      */
      ...
      if (expr.type === 'tripleConstraint') {
        ...
        var value = "valueClassRef" in expr ?
          ...
          expr.value;
        var tp = new TripleConstraint(expr.predicate, expr.inverse, expr.negated, value);
        ...
        var repeated = _repeat(group, expr.min, expr.max);
        return expr.semAct ? new SemActs(repeated, expr.semAct) : repeated;
      }
  ...
  this.compileShape = function (shape, triplePatterns, predicateToTriplePatterns) {
    ...
    return {
      ...
      ast: compileShapeToAST(shape.expression)
    };
  };
