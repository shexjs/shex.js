...
function _diagnose (db, point, expr) { // , tripleConstraints, predicateToTripleConstraints
  ...
  if (expr.type === 'tripleConstraint') {
    ...
    var incoming = indexNeighborhood(db.findByIRI(null, null, point, null).sort(bySubject));
      ...
      var index = expr.inverse ? incoming : outgoing;

      // get triples matching predciate
      var matchPredicate = index.byPredicate[expr.predicate];
    if (matchPredicate === undefined && min > 0) {
      ret.push({ type: "MissingProperty", property: expr.predicate, value: expr.value });
    } else {
      // strip to triples matching value constraints (apart from @<someShape>)
      var matchConstraints = _triplesMatchingPattern(matchPredicate, expr);
      if (matchConstraints.length < min)
        ret.push({ type: "MissingProperty", property: expr.predicate, value: expr.value });
      else if (matchConstraints.length > max)
        ret.push({ type: "MissingProperty", property: expr.predicate, value: expr.value });
    }
  }
    ...
    expr.expressions.forEach(function (nested, ord) {
      ret = ret.concat(_diagnose(db, point, nested));
    });
  }
  ...
  if (['someOf', 'group'].indexOf(expr.type) !== -1 &&
...
function _compileShapeToAST (expression, tripleConstraints, schema) {
  ...
  function _compileExpression (expr, schema) {
    ...
    */
    ...
    if (expr.type === "tripleConstraint") {
      // predicate, inverse, negated, value, annotations, semActs, min, max
      ...
      container = new Group(expr.expressions.map(function (e) {
        return _compileExpression(e, schema);
      }));
      repeated = _repeat(container, expr.min, expr.max);
      return expr.semActs ? new SemActs(repeated, expr.semActs) : repeated;
