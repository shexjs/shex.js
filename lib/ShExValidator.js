          ...
          if (!(valueExpr.values.some(function (valueConstraint) {
            if (typeof valueConstraint === "object" && !("value" in valueConstraint)) { // isTerm me -- strike "value" in
              ...
              function normalizedTest (val, ref, func) {
                var l = val, r = ref;
                if (val.startsWith("\"")) {
                  l = valueConstraint.type === "LanguageStemRange" ? N3Util.getLiteralLanguage(val) : N3Util.getLiteralValue(val);
                  if (!(typeof ref === "object" && "value" in ref) && valueConstraint.type !== "LanguageStemRange" && valueConstraint.type !== "LiteralStemRange") {
                    validationError("literal " + val + " not comparable with non-literal " + ref);
                  }
                  r = valueConstraint.type === "LanguageStemRange" || valueConstraint.type === "LiteralStemRange" ? ref : ref.value;
                } else if (["LanguageStemRange", "LiteralStemRange"].indexOf(valueConstraint.type) !== -1) {
                  validationError("nonliteral " + val + " not comparable with literal " + JSON.stringify(ref));
                }
                return func(l, r);
              }
