...
function ShExValidator(schema, options) {
  ...
  this.validate = function (db, point, shapeLabel, depth) {
    ...
    if ("startActs" in schema) { ret.startActs = schema.startActs; }
    return ret;
  };
   ...
   */
  this._triplesMatchingValueClass = function (triples, v, inverse) {
    var misses = [];
    var hits = [];
    triples.forEach(function (t) {
      ...
      function validationError () {
        ...
        errors.push("Error validating " + t.object + " as " + JSON.toString(v) + ": " + errorStr);
      }
      var value = inverse ? t.subject : t.object;
      ...
      if (!v.reference && !v.nodeKind && !v.values && !v.datatype) {
        ...
        if (v.values) {
          if (v.values.indexOf(t.object) !== -1) {
            ...
            if (!(v.values.some(function (valueConstraint) {
              if (typeof valueConstraint === "object") {
                ...
                if (typeof valueConstraint.stem === "object") {
                  ...
                  if (!(t.object.startsWith(valueConstraint.stem))) {
                    return false;
                  }
                }
      ...
      Object.keys(numericValueTests).forEach(function (test) {
        if (test in v) {
          if (numeric) {
            ...
            validationError("facet violation: numeric facet " + test + " can't apply to " + t.object);
          }
        }
      });
      ...
      Object.keys(decimalLexicalTests).forEach(function (test) {
        if (test in v) {
          if (numeric === XSD + "integer" || numeric === XSD + "decimal" || numeric === XSD + "float") {
            if (!decimalLexicalTests[test](""+numericParsers[numeric](label, validationError), v[test])) {
              validationError("facet violation: expected " + test + " of " + v[test] + " but got " + t.object);
            }
          } else {
            validationError("facet violation: numeric facet " + test + " can't apply to " + t.object);
