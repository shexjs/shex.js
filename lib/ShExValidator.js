...
function ShExValidator_constructor(schema, options) {
  ...
  this._validateShapeDecl = function (db, point, shapeExpr, shapeLabel, depth, tracker, seen, subgraph) {
    ...
    if (effectiveExpr.type === "ShapeDecl") {
      effectiveExpr = effectiveExpr.shapeExpr;
    }
  ...
  this._validateShape = function (db, point, regexEngine, shape, shapeLabel, depth, tracker, seen, subgraph) {
    ...
    var tripleList = constraintList.reduce(function (ret, constraint, ord) {
      ...
      function _errorsByShapeExpr (focus, shapeExpr) {
        var sub = _ShExValidator._validateShapeDecl(db, focus, shapeExpr, shapeLabel, depth, tracker, seen);
        return "errors" in sub ? sub.errors : [];
      }
      ...
      visitor.visitShape = function (shape, label) {
        ...
        if ("extends" in shape) {
          ...
          shape.extends.forEach(se => {
            // se is an ShapeExpression se with projection te.
            ...
            var te = maybe(EACH, [
              maybe(EACH, Object.keys(ins).map(p => ins[p])),
              maybe(EACH, Object.keys(outs).map(p => outs[p]))
            ]);
            te.onShapeExpression = se;
            tes.push(te);
          })
        }
        if ("expression" in shape)
          tes.push(shape.expression);
        if (tes.length)
          ret.expression = maybe(EACH, tes);
        return ret;
      }
    ...
    function mergeEffectiveTripleConstraints (expr, ins, outs) {
      ...
      visitor.visitTripleConstraint = function (expr) {
        ...
        idx[expr.predicate] = {
          ...
          min: Math.max(expr.predicate in idx ? idx[expr.predicate].min : 1, "min" in expr ? expr.min : 1),
          max: Math.min(expr.predicate in idx ? idx[expr.predicate].max : 1, "max" in expr ? expr.max : 1),
