...
function ShExValidator(schema, options) {
  ...
  this._validateShape = function (db, point, regexEngine, shape, shapeLabel, depth, seen, uniques) {
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      var possibleRet = { type: "ShapeTest", node: point, shape: shapeLabel };
  ...
  this._errorsMatchingNodeConstraint = function (value, valueExpr, recurse) {
    ...
    if ("pattern" in valueExpr) {
      var pat = valueExpr.pattern;
      var excludesPat = Math.random().toString(36).substring(7);
      pat = pat.replace(/\[\^/g, excludesPat);
      pat = pat.replace(/\^/g, "\\^").replace(/\$/g, "\\$");
      pat = pat.replace(new RegExp(excludesPat, 'g'), "[^", "g");
