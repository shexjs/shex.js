...
function ShExValidator(schema, options) {
  ...
  this.validate = function (db, point, shapeLabel, depth) {
   ...
   function _log () {
    ...
    var neighborhood = outgoing.triples.concat(incoming.triples);

    var tripleToConsraintList = constraintList.reduce(function (ret, constraint, ord) {
      ...
      var matchPredicate = constraint.inverse ?
        incoming.byPredicate[constraint.predicate] :
        outgoing.byPredicate[constraint.predicate] ||
      ...
      matchConstraints.forEach(function (t) {
        ret[neighborhood.indexOf(t)].push(ord);
      });
      return ret;
      // var x = _triplesMatchingDependentShape(db, matchConstraints, constraint, depth);
    }, _seq(neighborhood.length).map(function () { return []; })); // start with [[],[]...]

    // tripleToConsraintList.forEach(function (candidate) { // limit to OPEN predicates
    //   candidate.push(ε);
    // });

    var ret = null;
    var r = new RegExp("^"+compiledShape.regexp+"$"); // ((0 )*(1 )+|(2 )(3 ))
    var xp = crossProduct(tripleToConsraintList);
    while (tripleToConsraintList.length !== 0 && xp.next() && ret === null)
      xp.do(function () {
        // caution: early returns
        var usedTriples = [];
        var constraintMatchCount = _seq(neighborhood.length).map(function () { return 0; }); /* Map[Triple, Int] */
        // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/arguments#Description says to iterate instead of assigning 
...
debugger;
    ...
    var alts = []; // cartesianProductOf(tripleToConsraintList)
