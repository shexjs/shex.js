    ...
    var results = candidates.reduce((ret, label) => {
      ...
      function effectiveShapeExpression (shape, tcs) {
        ...
        function projectionAsTripleExpression (expr) {
          ...
          visitor.visitShape = function (shape, label) {
            // add extends[].expression and .expression to tes.
            // return {type: "Shape"}
            // return extend(shape, tes);
            var ses = [];
            if ("extends" in shape) {
              ses =
                shape.extends.map(l =>
            ...
            if ("expression" in shape) {
              var newTe = ShExUtil.Visitor().visitExpression(shape.expression);
              if (ses.length > 0)
                newTe.scopedShapeExpression = maybe(AND, ses);
              tes.push(newTe);
            }
