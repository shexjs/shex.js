...
function ShExValidator(schema, options) {
  ...
  this._expect = options.lax ? noop : expect; // report errors on missing types.
  ...
  this.getAST = function () {
    return {
      ...
      shapes: Object.keys(this._shapeCompilationByLabel).reduce(function (ret, label) {
        ret[label] = {
          type: "ASTshape",
          expression: _compileShapeToAST(_ShExValidator.schema.shapes[label].expression, [])
        };
        return ret;
      }, {})
    };
  ...
  this.compileShape = function (shape) {
    ...
    return {
      ...
      optimize: optimize
      //ast: _compileShapeToAST(shape.expression, []) // @@ make sure this ends up the same as _ShExValidator._shapeCompilationByLabel[label].tripleConstraints
    };
  };
  ...
  this.validate = function (db, point, shapeLabel, depth) {
	...
	return _ShExValidator.validate(db, point, shapeLabel, depth);
      ...
      var subVal = schema.shapes[shapeLabel].expression ?
        _toVal(schema.shapes[shapeLabel].expression, _constraintToTriples(), constraintList, neighborhood, db, recurse, depth) :
        {};
      if (subVal === null)
        continue;
 ...
 */
function _compileShapeToAST (expression, tripleConstraints) {
...
function _compileRegexp (expr, tripleConstraints, predicateToTripleConstraints, optimize) {
  ...
  if (expr.type === 'tripleConstraint') {
    ...
    // @@ inverse, predicate, min, max, semAct, AND
  }
    ...
    regexp += _compileRegexp(expr.expressions, tripleConstraints, predicateToTripleConstraints, optimize);
  }
  ...
  if (['someOf', 'group', 'wrapper'].indexOf(expr.type) !== -1 &&
 ...
 */
function _toVal (expr, constraintToTriples, constraintList, neighborhood, db, validate, depth) { // consumes constraintToTriples
  if (expr.type === 'tripleConstraint') {
    var constraintNo = constraintList.indexOf(expr);
    var take = expr.max === undefined ? 1 : expr.max;
    var ret = {
    ...
    do {
      ...
      for (var ord = 0; ord < expr.expressions.length; ++ord) {
        ...
        var addMe = _toVal(nested, constraintToTriples, constraintList, neighborhood, db, validate, depth);
        if (addMe === null) {
          if (expr.type === 'group')
            return null;
          else
    ...
    do {
      expr.expressions.forEach(function (nested, ord) {
        regexp += added = _toVal(nested, constraintToTriples, constraintList, neighborhood, db, validate, depth)
        if (added === null)
          return null;
      });
    } while (added !== "");
