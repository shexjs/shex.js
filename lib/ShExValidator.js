...
function ShExValidator(schema, options) {
  ...
  this._toVal = function (expr, constraintToTriples, constraintList, neighborhood, db, validate, depth, seen) { // consumes constraintToTriples
    ...
    if (expr.type === "TripleConstraint") {
      ...
      while (constraintToTriples[constraintNo].length && (take === "*" || take--)) {
        ...
        if (!_validator.semActHandler.dispatchAll(expr.semActs || [], testedTriple))
  ...
  this._triplesMatchingValueExpr = function (triples, valueExpr, inverse) {
    ...
    if (valueExpr.type === "ValueClass") {
      var ret = this._triplesMatchingValueClass (triples, valueExpr, inverse);
      return ret;
    } else if (valueExpr.type === "ValueRef") {
      return this._triplesMatchingValueExpr (triples, schema.valueExprDefns[valueExpr.valueExprRef].valueExpr, inverse);
    } else if (valueExpr.type === "ValueOr") {
      return valueExpr.valueExprs.reduce(function (ret, nested) {
        var r = _ShExValidator._triplesMatchingValueExpr (triples, nested, inverse);
        return {hits: ret.hits.concat(r.hits), misses: ret.misses.concat(r.misses)};
      }, {hits: [], misses: []});
    } else if (valueExpr.type === "ValueAnd") {
      return valueExpr.valueExprs.reduce(function (ret, nested, iter) {
        var r = _ShExValidator._triplesMatchingValueExpr (triples, nested, inverse);
        var hits = iter === 0 ? r.hits : [];
        for (var i = 0; i < ret.hits.length; i++) {
          var triple = ret.hits[i];
          if (r.hits.indexOf(triple) !== -1) {
            hits.push(triple);
          }
        }
        return {hits: hits, misses: ret.misses.concat(r.misses)};
      }, {hits: [], misses: []});
    } else {
  ...
  /* _triplesMatchingValueClass - return the triples matching the value
   * class without checking shape references.
   */
  this._triplesMatchingValueClass = function (triples, valueExpr, inverse) {
    var misses = [];
    var hits = [];
