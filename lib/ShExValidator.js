...
function ShExValidator(schema, options) {
  ...
  //is._validatePoint = function (db, point, compiledExpression, nfa, shape, shapeLabel, depth, seen, threads) {
  this._validatePoint = function (graph, node, outerExpression, constraintList, constraintToTripleMapping, tripleToConstraintMapping) {
    var _this = this;
    var newThreads = [];
    ...
    function validateExpr (expr, thread) {
      ...
      function validateRept (type, val) {
        ...
        //   ret.annotations = expr.annotations;
        // if ("semActs" in expr)
        ...
        //     return repeated < min ? sub : ret;
        //   else
        //     ret = ret.concat(sub);
        //     // ret.expressions.push(sub);
        // }
        // return ret;
        ...
        // var newThreads = [th];
        // for (var e = 0; e < expr.expressions.length; ++e) {
        //   var nested = expr.expressions[e];
        //   var inner = [];
        //   for (var t = 0; t < newThreads.length; ++t) {
        //     var newt = newThreads[t];
        //     inner = inner.concat(validateExpr(nested, newt));
        //   }
        //   newThreads = inner;
        // }
      ...
      if (expr.type === "TripleConstraint") {
        ...
        var ret = [];
        var errors = thread.errors.slice();
        ...
        if (passed) {
          do {
            ret.push({
              ...
              errors: errors,
              matched: matched.concat({expr: expr, triples: taken})
            });
          } while ((function () {
          ...
          errors.push({
            type: negated ? "UnexpectedProperty" : "MissingProperty",
        ...
        return validateRept("eachOfSolutions", (th) => {
          ...
          return expr.expressions.reduce((newThreads, nested) => {
            return newThreads.reduce((newts, newt) => {
              return newts.concat(validateExpr(nested, newt));
            }, []);
          }, [th]);
        });
      }
    ...
    if (!outerExpression)
      return { avail: {}, errors: [], matched: [] }; // vapid match if no expression
    var ret = validateExpr(outerExpression, {avail:[], matched:[], errors:[]});
    // console.log(JSON.stringify(ret));
    var longerChosen = ret.reduce((ret, elt) => {
      ...
      Object.keys(unmatchedTriples).forEach(t => {
        elt.errors.push({
          type: "MaxCardinalityViolation",
          triple: t,
          constraint: unmatchedTriples[t]
        });
      });
      return ret !== null ? ret : Object.keys(unmatchedTriples).length ? null : elt;
      var matchedAll =
      ...
      if (!matchedAll)
        elt.errors.push("didn't match some triple(s)");
      return ret !== null ? ret : matchedAll ? elt : null;
    }, null)
    return longerChosen === null ? ret : [longerChosen];
  ...
  this.compileNFA = function (expression) {
    ...
    function rbenx_match (graph, node, trace, constraintList, constraintToTripleMapping, tripleToConstraintMapping) {
      var _this = this;
  ...
  this.validate = function (db, point, shapeLabel, depth, seen) {
    ...
    if (shapeExpr.type === "Shape") {
      ret = this._validateShape(db, point, compiledExpression, nfa,
      ...
      ret = {
        ...
        shapeExpr.shapeExprs.map((shape, ord) => {
          return this._validateShape(db, point, compiledExpression[ord], nfa[ord],
  ...
  this._validateShape = function (db, point, compiledExpression, nfa, shape, shapeLabel, depth, seen) {
    var _ShExValidator = this;
...
// !! copied
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      var fromValidatePoint = this._validatePoint(db, point, shape.expression, constraintList, _constraintToTriples(), tripleToConstraintMapping);
      var nfaTrace = [];
      var fromNFA = nfa.match(db, point, nfaTrace, constraintList, _constraintToTriples(), tripleToConstraintMapping);
      if ("errors" in fromNFA !== (fromValidatePoint.length > 0 && fromValidatePoint[0].errors.length > 0))
         { throw Error(JSON.stringify(fromValidatePoint) + " vs " + JSON.stringify(fromNFA)); }
      if (!("errors" in fromNFA) && fromNFA.constructor === Array) { // {} for empty result
        ...
        fromNFA = _ShExValidator.matchedToResult(fromNFA, neighborhood, _recurse, this.semActHandler);
        if ("semActs" in shape) {
