...
function ShExValidator(schema, options) {
  ...
  function _triplesMatchingPattern (triples, triplePattern) {
    ...
    triples.forEach(function (t) {
      ...
      if (!v.reference && !v.nodeKind && !v.values && !v.datatype) {
        ...
        if (v.reference && v.values  ) runtimeError("found both reference and values in "  +triplePattern);
        if (v.reference && v.datatype) runtimeError("found both reference and datatype in "+triplePattern);
        if (v.datatype  && v.values  ) runtimeError("found both datatype and values in "   +triplePattern);
  ...
  function _triplesMatchingDependentShape (db, triples, triplePattern) {
    var ret = [];
    triples.forEach(function (t) {
      var errors = [];
      ...
      if (!v.reference && !v.nodeKind && !v.values && !v.datatype) {
        ...
        if (v.reference) {
          if (typeof(v.reference) === "object") {
            ...
            if (_ShExValidator.validate(db, t.object, v.reference) === null)
    ...
    l.forEach(function (t) {
      if (VERBOSE)
        t.toString = function () { // for printf debugging
          function brack (n) {
            return N3Util.isLiteral(n) ? n : N3Util.isBlank(n) ? n : "<" + n + ">";
          }
        return brack(t.subject) + " " + brack(t.predicate) + " " + brack(t.object) + " .";
      };
      var p = t.predicate;
      if (!(p in i))
        i[p] = [];
  ...
  this.validate = function (db, point, shapeLabel) {
    ...
    this._triplePatternsByLabel[shapeLabel].forEach(function (tp, tpIndex) {
      ...
      // var x = _triplesMatchingDependentShape(db, matchConstraints, tp);
    });
    // console.log(outgoingCandidates);

    var xp = crossProduct(outgoingCandidates);
    if (VERBOSE) console.log("<" + shapeLabel + ">");
    var ret = null;
    while (xp.next() && ret === null) xp.
      do(function () { // caution, early returns
        ...
        var tripleToConstraintMapping = []; // = Array.prototype.slice.call(arguments)
        for (var i = 0; i < arguments.length; ++i)
          tripleToConstraintMapping.push(arguments[i]);

        if (VERBOSE) console.log("trying " + shapeLabel + " for " + tripleToConstraintMapping.join(' '));
        var byConstraint = tripleToConstraintMapping.slice().sort().join(" ")+" "; // 0 0 1 3
        var solution = byConstraint.match(r);
        if (!solution)
          return;
        if (VERBOSE) console.log("post-regexp " + tripleToConstraintMapping.join(' '));
        for (var i = 0; i < outgoing.length; ++i) {
          ...
          //   return;
          if (_triplesMatchingDependentShape(db, [triple], triplePattern).length === 0)
            return;
        }
        if (VERBOSE) console.log("final " + tripleToConstraintMapping.join(' '));
        ret = { expression: schema.shapes[shapeLabel].expression, triples: tripleToConstraintMapping.map(function (m, ord) {
          if (VERBOSE) {
            delete outgoing[ord].toString;
          }
          return outgoing[ord];
        })};
