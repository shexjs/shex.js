...
function ShExValidator(schema, options) {
  ...
  this._validatePoint = function (graph, node, outerExpression, constraintList, constraintToTripleMapping, tripleToConstraintMapping, neighborhood) {
          ...
          for (var t = 0; t < newThreads.length; ++t) {
            ...
            if (sub[0].errors.length === 0) {
              sub.forEach(newThread => {
                expression.solutions.push(newThread.solution);
                delete newThread.solution;
                newThread.expression = expression;
              });
            }
            if (sub.length === 0 /* min:0 */ || sub[0].errors.length > 0)
        ...
        return validateRept("someOfSolutions", (th) => {
          ...
          expr.expressions.forEach(nested => {
            ...
            if (sub[0].errors.length === 0) {
              ...
              sub.forEach(newThread => {
                solution.expressions.push(newThread.expression);
                delete newThread.expression;
                newThread.solution = solution;
              });
            } else
    ...
    if (!outerExpression)
      return startingThread.matched; // vapid match if no expression
