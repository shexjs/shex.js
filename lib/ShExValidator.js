...
function ShExValidator(schema, options) {
  ...
  this._vpEngine = function (shape) {
    var outerExpression = shape.expression;
    return {
      match: function (graph, node, constraintList, constraintToTripleMapping, tripleToConstraintMapping, neighborhood, trace) {
        ...
        function validateExpr (expr, thread) {
          ...
          function validateRept (type, val) {
            ...
            if (newThreads.length > 0 && newThreads[0].errors.length === 0 &&
                !_ShExValidator.semActHandler.dispatchAll(expr.semActs || [], "???"))
        ...
        return longerChosen !== null ? longerChosen.expression :
          ret.length > 1 ? {
            type: "PossibleErrors",
      ...
      finish: function (fromValidatePoint, constraintList, neighborhood, recurse, semActHandler) {
        function _dive (solns) {
          if (solns.type === "someOfSolutions" ||
            ...
            solns.solutions = solns.solutions.map(x => {
              ...
              var subErrors = _ShExValidator._errorsMatchingValueExpr(expr.inverse ? t.subject : t.object, expr.valueExpr, expr.inverse, function (focus, shapeLabel) {
   ...
   */
  this.compileNFA = function (shape) {
    ...
    function rbenx_match (graph, node, constraintList, constraintToTripleMapping, tripleToConstraintMapping, neighborhood, trace) {
      var _this = this;
    ...
    function matchedToResult (matched, constraintList, neighborhood, recurse, semActHandler) {
      var last = [];
      var errors = [];
      var skips = [];
  ...
  this.validate = function (db, point, shapeLabel, depth, seen) {
    ...
    if (shapeExpr.type === "Shape") {
      ret = this._validateShape(db, point, this._vpEngine(shapeExpr),
                                shapeExpr, shapeLabel, depth, seen, compiledExpression.tripleConstraints, null);
    } else {
      ret = {
        "type": "ShapeAnd",
        "solutions":
        shapeExpr.shapeExprs.map((shape, ord) => {
          return this._validateShape(db, point, this._vpEngine(shape),
                                     shape, shapeLabel, depth, seen, compiledExpression[ord].tripleConstraints, ord);
        })
      };
    }
    delete seen[seenKey];
  ...
  this._validateShape = function (db, point, regexEngine, shape, shapeLabel, depth, seen, constraintList, ord) {
    var _ShExValidator = this;
...
// !! copied
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      var results = regexEngine.match(db, point, constraintList, _constraintToTriples(), tripleToConstraintMapping, neighborhood, null);
      ...
      if (false) {// testing parity between two engines
        var nfa = schema.shapes[shapeLabel].type === "Shape" ?
            _ShExValidator._NFAByLabel[shapeLabel] :
            _ShExValidator._NFAByLabel[shapeLabel][ord];
        var fromNFA = nfa.match(db, point, constraintList, _constraintToTriples(), tripleToConstraintMapping, null);
        if ("errors" in fromNFA !== "errors" in results)
          { throw Error(JSON.stringify(results) + " vs " + JSON.stringify(fromNFA)); }
        // if (!("errors" in fromNFA)) // duplicates semantic action results
        //   fromNFA = nfa.finish(fromNFA, constraintList, neighborhood, _recurse, this.semActHandler);
      }
      if (!("errors" in results))
        results = regexEngine.finish(results, constraintList, neighborhood, _recurse, this.semActHandler);
      if ("errors" in results) {
      ...
      if (!this.semActHandler.dispatchAll(shape.semActs || [], results))
        continue; // some semAct aborted
      _log("final " + usedTriples.join(" "));

      ret = { type: "test", node: point, shape: shapeLabel, solution: results }; // expression: expr, triples: usedTriples
  ...
  Object.keys(schema.shapes).forEach(function (label) {
    ...
    _ShExValidator._NFAByLabel[label] =
      shapeExpr.type === "Shape" ?
      _ShExValidator.compileNFA(shapeExpr) :
      shapeExpr.shapeExprs.map(shape => {
        return _ShExValidator.compileNFA(shape);
      });
  });
