...
var ShExLoader = require("./ShExLoader");
...
function findNodesAndValidate (loaded, validateParms, validateOpts, cmdOpts) {
  cmdOpts = cmdOpts || {};
  ...
  function someIRInode () {
    ...
    for (var i = 0; i < triples.length; ++i)
      if (N3Util.isIRI(triples[i].subject))
  ...
  function _makeShapeMap () {
    // resolve relative shape and focus node names against the first schema and data source respectively.
    var node = validateParms["node-type"] ?
        someNodeWithType(ShExUtil.parsePassedNode(validateParms["node-type"], loaded.dataMeta[0],
                                                  null, knownType, loaded.data.prefixes)) :
        ShExUtil.parsePassedNode(validateParms.node, loaded.dataMeta[0], someIRInode,
                                 knownNode, loaded.data.prefixes);
    var shape, shapeLabel;
    ret.mode = node;
    if (!validateParms.shape && loaded.schema.start) {
      shape = loaded.schema.start;
      shapeLabel = "- start -";
      ret.shape = shapeLabel;
    } else {
      var found = ShExUtil.parsePassedNode(validateParms.shape, loaded.schemaMeta[0],
                                           someShape, knownShape, loaded.schema.prefixes);
      if (found === ShExUtil.NotSupplied || found === ShExUtil.UnknownIRI)
        throw Error("shape " + validateParms.shape + " not defined" + optsStr(validateParms.shape, loaded.schema.shapes));
      // shape = { type: "ShapeRef", reference: found };

      shape = found;
      shapeLabel = validateParms.shape;
      if (validateParms.shape === undefined)
        console.warn("Guessing shape " + found);
    }

    if (node === ShExUtil.NotSupplied)
      throw Error("node not defined and no default found" +
                  ("node-type" in validateParms ?
                   " looking for type " + validateParms["node-type"] :
                   ""));
    if (node === ShExUtil.UnknownIRI) {
      console.warn("node " + validateParms.node + " not found in data" +
                   optsStr("node" in validateParms ? validateParms.node : node, loaded.data._entities));
      node = validateParms.node;
    }
  ...
  var shapeMap = "map" in validateParms ? validateParms.map : _makeShapeMap();
  if (cmdOpts.verbose) {
  ...
  ret.results = runValidator(loaded.data, shapeMap, loaded.schema, validateOpts, cmdOpts);
...
function runValidator (graph, shapeMap, schema, validateOpts, cmdOpts) {
  // prepare validator
  var validator = ShExValidator_constructor(schema, validateOpts);
...
if (typeof require !== "undefined" && typeof exports !== "undefined")
  module.exports = { // node environment
    ...
    findNodesAndValidate: findNodesAndValidate
  };
else
