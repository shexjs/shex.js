...
function ShExValidator(schema, options) {
  ...
  this._validateShape = function (db, point, regexEngine, shape, shapeLabel, depth, seen, constraintList, ord) {
    ...
    var errors = this._errorsMatchingValueClass(point, shape, false, null);
    var ret = null;
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      function _testExpr (term, valueExpr, inverse, recurse) {
        return _ShExValidator._errorsMatchingValueExpr(term, valueExpr, inverse, recurse)
      }
