...
function ShExValidator(schema, options) {
  ...
  var Split   = "<span class='keyword' title='Split (choice)'>|</span>";
  var Match   = "<span class='keyword' title='Match'>␃</span>";
  var Rept    = "<span class='keyword' title='Repeat'>×</span>";
  var Inf     = "<span class='keyword' title='Infinity'>∞</span>";
  ...
  this.compileNFA = function (expression) {
    ...
    function walkExpr (expr) {
      ...
      if (expr.type === "TripleConstraint") {
        ...
        var included = schema.shapes[expr.include].expression;
        return walkExpr(included);
      }

      runtimeError("unexpected expr type: " + expr.type);
    };

    function state (c, outs, negated) {
    ...
    function rbenx_match (graph, node, trace) {
      var _this = this;
      ...
      function resetRepeat (thread, repeatedState) {
        ...
        return {state:thread.state, repeats:trimmedRepeats};
      }
      function incrmRepeat (thread, repeatedState) {
        ...
        return {state:thread.state, repeats:incrmedRepeats};
      }
      ...
      function addstate (list, stateNo, thread, seen) {
        ...
        if (s.c === Split) {
          ...
          var repetitions = thread.repeats[stateNo]+1;
          ...
          list.push({state:stateNo, repeats:thread.repeats});
        }
      }
      ...
      addstate(clist, startNo, {repeats:{}}); // a thread which repeats no states
      for (var i = 0; i < str.length; ++i) {
        var c = str[i];
        nlist = [];
        if (trace)
          trace.push({c:c, threads:[]});
        for (var threadno = 0; threadno < clist.length; ++threadno) {
          ...
          if (matchAtom(state.c, c, state.negated)) {
        ...
        if (nlist.length === 0)
          return reportError(localExpect(clist, _this.states), str, i);
        var t = clist;
        clist = nlist;
        nlist = t;
      }
      var ret = clist.reduce((ret, elt) => {
        return ret || elt.state === _this.end;
      }, false);
      if (ret === false)
        return reportError(localExpect(clist, _this.states), str, i);
      return Solution();
   ...
   */
  this._toVal = function (expr, constraintToTriples, constraintList, neighborhood, db, validate, depth, seen) { // consumes constraintToTriples
    var _ShExValidator = this;
    if (expr.type === "TripleConstraint") {
      ...
      while (constraintToTriples[constraintNo].length && (take === "*" || take--)) {
        ...
        //                      expr.valueExpr.reference, depth + 1, seen);
        //   if (sub === null)
        //     return null;
        //   testedTriple.referenced = sub;
        // }
        var errors = _ShExValidator._errorsMatchingValueExpr(expr.inverse ? triple.subject : triple.object, expr.valueExpr, expr.inverse, function (focus, shape) {
          var sub = validate(db, focus, shape, depth + 1, seen);
          if ("errors" in sub) {
            // console.dir(sub);
            return [{ type: "ReferenceError", focus: focus, shape: shape, errors: sub }];
          }
          testedTriple.referenced = sub; // !!! needs to aggregate errors and solutions
          return [];
        });
        if (errors.length)
          return { type: "ExpressionError", errors: errors };
        if (!_ShExValidator.semActHandler.dispatchAll(expr.semActs || [], testedTriple))
          return { type: "SemActFailure", errors: [{ type: "UntrackedSemActFailure" }] } // some semAct aborted
      ...
      do {
        ...
        for (var ord = 0; ord < expr.expressions.length; ++ord) {
          ...
          var addMe = this._toVal(nested, constraintToTriples, constraintList, neighborhood, db, validate, depth, seen);
          if ("errors" in addMe) {
            if (expr.type === "EachOf")
              return { type: "EachOfFailure", errors: [addMe] };
            else
              failures.push(addMe);
          } else if (addMe.solutions.length !== 0)
            added.push(addMe);
        }
        if (failures.length === expr.expressions.length)
          return { type: "SomeOfFailure", errors: failures };
        if (added.length !== 0) {
          solutions.push({
            type: expr.type === "SomeOf" ? "someOfSolution" : "eachOfSolution",
            expressions: added
          });
        } else {
          done = true;
      ...
      ret = {
        type: expr.type === "SomeOf" ? "someOfSolutions" : "eachOfSolutions", // note the 's'
        solutions: solutions
      };
      if (ret && "min" in expr) { ret.min = expr.min; }
      if (ret && "max" in expr) { ret.max = expr.max; }
      if (ret && "annotations" in expr) { ret.annotations = expr.annotations; }
      if (ret && !this.semActHandler.dispatchAll(expr.semActs || [], ret))
        ret = { type: "SemActFailure", errors: [{ type: "UntrackedSemActFailure" }] };
      if (ret && "semActs" in expr) { ret.semActs = expr.semActs; }
      return ret;
    }
      ...
      var included = schema.shapes[expr.include].expression;
      return this._toVal(included, constraintToTriples, constraintList, neighborhood, db, validate, depth, seen);
    }

    else runtimeError("unexpected expr type: " + expr.type);
    return ProgramFlowError;
  }
  ...
  this.validate = function (db, point, shapeLabel, depth, seen) {
    ...
    var ret = this._validateExpression(db, point, this._shapeCompilationByLabel[shapeLabel],
                                       schema.shapes[shapeLabel], shapeLabel, depth, seen);
    return ret;
  }

  this._validateExpression = function (db, point, compiledExpression, shape, shapeLabel, depth, seen) {
    var _ShExValidator = this;
    if (seen === undefined)
      seen = {};
    ...
    var valueExpr = schema.shapes[shapeLabel];
    var value = point;
...
// !! copied
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      function recurse (db, point, shapeLabel, depth, seen) {
        return _ShExValidator.validate(db, point, shapeLabel, depth, seen);
      }
      var subVal = shape.expression ?
        _ShExValidator._toVal(shape.expression, _constraintToTriples(), constraintList, neighborhood, db, recurse, depth, seen) :
        {};
      if ("errors" in subVal)
        continue;
      if (!this.semActHandler.dispatchAll(shape.semActs || [], subVal))
        continue; // some semAct aborted
      if ("semActs" in shape) { subVal.semActs = shape.semActs; }
    ...
    if (ret === null && this.options.diagnose) {
      ret = {
        ...
        node: point,
        shape: shapeLabel,
        errors: misses.length > 0 ? misses.map(function (miss) {
          return {
            type: "TypeMismatch",
            triple: neighborhood[miss.tripleNo],
            constraint: constraintList[miss.constraintNo]
          };
        }) : this._diagnose(db, point, shape.expression, shapeLabel)
      };
    }
   ...
   */
  this._diagnose = function (db, point, expr, shapeLabel) { // , tripleConstraints, predicateToTripleConstraints
    var _ShExValidator = this;
    var errors = [];
    ...
    if (expr.type === 'TripleConstraint') {
      errors.push({ type: "MissingProperty", property: expr.predicate, valueExpr: expr.valueExpr });
    }

    else if (expr.type === 'SomeOf') {
      var disjuncts = [];
      expr.expressions.forEach(function (nested, ord) {
        var compiledExpression = _ShExValidator.compileExpression(nested);
        var shape = {
          type: "Shape",
          expression: nested
        };
        var r = _ShExValidator._validateExpression(db, point, compiledExpression, shape, shapeLabel);
        if ("errors" in r)
          disjuncts = disjuncts.concat(r.errors);
      });
      errors.push({ type: "DisjuctionFailure", disjuncts: disjuncts });
    }
  ...
  Object.keys(schema.shapes).forEach(function (label) {
    _ShExValidator._NFAByLabel[label] =
      _ShExValidator.compileNFA(schema.shapes[label].expression);
  });
