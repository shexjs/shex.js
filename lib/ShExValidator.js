...
function ShExValidator(schema, options) {
  ...
  this.matchedToResult = function (matched, neighborhood, recurse, semActHandler) {
    ...
    function _removeNulls (matched) {
      if ("solutions" in matched) {
        for (var solnNo = 0; solnNo < matched.solutions.length; ++solnNo) {
          if ("referenced" in matched.solutions[solnNo]) {
            _removeNulls(matched.solutions[solnNo].referenced);
          } else if ("expressions" in matched.solutions[solnNo]) {
            for (var exprNo = 0; exprNo < matched.solutions[solnNo].expressions.length; ++exprNo) {
              if (matched.solutions[solnNo].expressions[exprNo] === null ||
                  matched.solutions[solnNo].expressions[exprNo] === undefined) // !!! undefined in validation/manifest#skipped
                matched.solutions[solnNo].expressions.splice(exprNo--, 1);
              else
                _removeNulls(matched.solutions[solnNo].expressions[exprNo]);
            }
          }
        }
      }
...
// !! copied
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      var nfaTrace = [];
      var fromNFA = _ShExValidator._NFAByLabel[shapeLabel].match(db, point, nfaTrace, constraintList, _constraintToTriples());
    ...
    if (ret === null && this.options.diagnose) {
      ...
      ret = {
        ...
        node: point,
        shape: shapeLabel,
        errors: missErrors.concat(partitionErrors.length === 1 ? partitionErrors[0].errors : partitionErrors) // this._diagnose(db, point, shape.expression, shapeLabel)
      };
    }
