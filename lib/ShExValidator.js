...
function ShExValidator(schema, options) {
  ...
  this.indexTripleConstraints = function (expression) {
    ...
    function indexTripleConstraints_dive (expr) {
      if (expr.type === "TripleConstraint")
        ...
        indexTripleConstraints_dive(schema.shapes[expr.include].expression);
   ...
   */
  this.validate = function (db, point, labelOrShape, depth, seen, uniques) {
    ...
    if (typeof labelOrShape !== "string")
      return this._validateShapeExpr(db, point, labelOrShape, "_: -start-", depth, seen, uniques);
  ...
  this._validateShapeExpr = function (db, point, shapeExpr, shapeLabel, depth, seen, uniques) {
	...
	node: point,
	shape: shapeLabel,
	solution: {} // regex matchers add .referenced to everything with a .solution
      };
    } else if (shapeExpr.type === "Shape") {
      return this._validateShape(db, point, regexModule.compile(schema, shapeExpr),
                                 shapeExpr, shapeLabel, depth, seen, uniques);
    } else if (shapeExpr.type === "ShapeRef") {
      return this._validateShapeExpr(db, point, schema.shapes[shapeExpr.reference], shapeExpr.reference, depth, seen, uniques);
    } else if (shapeExpr.type === "ShapeExternal") {
      return this.options.validateExtern(db, point, shapeLabel, depth, seen, uniques);
    } else if (shapeExpr.type === "ShapeOr") {
      var errors = [];
      for (var i = 0; i < shapeExpr.shapeExprs.length; ++i) {
        var nested = shapeExpr.shapeExprs[i];
        var sub = this._validateShapeExpr(db, point, nested, shapeLabel, depth, seen, uniques);
        if ("errors" in sub)
          errors.push(sub);
        else
      ...
      var sub = this._validateShapeExpr(db, point, shapeExpr.shapeExpr, shapeLabel, depth, seen, uniques);
      if ("errors" in sub)
      ...
      var passes = [];
      for (var i = 0; i < shapeExpr.shapeExprs.length; ++i) {
        var nested = shapeExpr.shapeExprs[i];debugger;
        var sub = this._validateShapeExpr(db, point, nested, shapeLabel, depth, seen, uniques);
        if ("errors" in sub)
  ...
  this._validateShape = function (db, point, regexEngine, shape, shapeLabel, depth, seen, uniques) {
    var _ShExValidator = this;
    ...
    var tripleList = constraintList.reduce(function (ret, constraint, ord) {
      ...
      function _errorsByShapeLabel (focus, shapeLabel) {
        var sub = _ShExValidator.validate(db, focus, shapeLabel, depth + 1, seen, uniques);
        return "errors" in sub ? sub.errors : [];
      }
      function _errorsByShapeExpr (focus, shapeExpr) {
        var sub = _ShExValidator._validateShapeExpr(db, focus, shapeExpr, depth, seen, uniques);
        return "errors" in sub ? sub.errors : [];
      }
      // strip to triples matching value constraints (apart from @<someShape>)
      var matchConstraints = _ShExValidator._triplesMatchingShapeExpr(matchPredicate, constraint.valueExpr, constraint.inverse, _ShExValidator.options.partition === "exhaustive" ? undefined : _errorsByShapeLabel, _ShExValidator.options.partition === "exhaustive" ? undefined : _errorsByShapeExpr);
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      function _recurse (point, shapeLabel) {
        return _ShExValidator.validate(db, point, shapeLabel, depth+1, seen, uniques);
      }
      function _direct (point, shapeExpr) {
        return _ShExValidator._validateShapeExpr(db, point, shapeExpr, shapeLabel, depth, seen, uniques);
      }
      ...
      var results = regexEngine.match(db, point, constraintList, _constraintToTriples(), tripleToConstraintMapping, neighborhood, _recurse, _direct, this.semActHandler, _testExpr, null, uniques);
      ...
      var possibleRet = { type: "ShapeTest", node: point, shape: shapeLabel, solution: results }; // expression: expr, triples: usedTriples
      if ("semActs" in shape &&
