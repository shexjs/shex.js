...
function ShExValidator(schema, options) {
  ...
  this.validate = function (db, point, shapeLabel, depth, seen) {
    ...
    if (!(shapeLabel in this.schema.shapes))
  ...
  this._validateShapeExpr = function (db, point, shapeExpr, shapeLabel, depth, seen) {
    if (shapeExpr.type === "NodeConstraint") {
      return this._validateShape(db, point, regexModule.compile(schema, shapeExpr),
                                 shapeExpr, shapeLabel, depth, seen); // !!!! mv _errorsMatchingValueClass(point, shape, null) into new function
      ...
      for (var i = 0; i < shapeExpr.shapeExprs.length; ++i) {
        ...
        if ("errors" in sub)
          ...
          return { type: "ShapeOrResults", solutions: sub };
      }
      return { type: "ShapeOrFailure", errors: errors };
    } else if (shapeExpr.type === "ShapeNot") {
      var sub = this._validateShapeExpr(db, point, shapeExpr.shape, shapeLabel, depth, seen);
      if ("errors" in sub)
          return { type: "ShapeOrResults", solutions: sub };
        else
  ...
  this._validateShape = function (db, point, regexEngine, shape, shapeLabel, depth, seen) {
    ...
    // _errorsMatchingValueClass(_, _, false, null) tests focus node's value constraints.
    var errors = this._errorsMatchingValueClass(point, shape, null);
    var ret = null;
    ...
    else if ("startActs" in schema && !this.semActHandler.dispatchAll(schema.startActs, null, startAcionStorage))
    ...
    var tripleList = constraintList.reduce(function (ret, constraint, ord) {
      ...
      function _errorsByShapeExpr (focus, shapeExpr) {
        var sub = _ShExValidator._validateShapeExpr(db, focus, shapeExpr, depth, seen);
        return "errors" in sub ? sub.errors : [];
      }
      // strip to triples matching value constraints (apart from @<someShape>)
      var matchConstraints = _ShExValidator._triplesMatchingValueExpr(matchPredicate, constraint.valueExpr, constraint.inverse, _ShExValidator.options.partition === "exhaustive" ? undefined : _errorsByShapeLabel, _ShExValidator.options.partition === "exhaustive" ? undefined : _errorsByShapeExpr);
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      function _testExpr (term, valueExpr, recurse, direct) {
        return _ShExValidator._errorsMatchingValueExpr(term, valueExpr, recurse, direct)
      }
      ...
      var possibleRet = { type: "test", node: point, shape: shapeLabel, solution: results }; // expression: expr, triples: usedTriples
      if ("semActs" in shape &&
  ...
  this._triplesMatchingValueExpr = function (triples, valueExpr, inverse, recurse, direct) {
    var _ShExValidator = this;
    var misses = [];
    var hits = [];
    triples.forEach(function (triple) {
      ...
      var errors = valueExpr === undefined ?
          [] :
          _ShExValidator._errorsMatchingValueExpr(value, valueExpr, recurse, direct);
      if (errors.length === 0) {
        hits.push(triple);
  ...
  this._errorsMatchingValueExpr = function (value, valueExpr, recurse, direct) {
    var _ShExValidator = this;
    if (valueExpr.type === "NodeConstraint") { // !!!! just needs to test node here.
      ...
      for (var i = 0; i < valueExpr.shapeExprs.length; ++i) {
        var nested = _ShExValidator._errorsMatchingValueExpr(value, valueExpr.shapeExprs[i], recurse, direct);
        if (nested.length === 0)
          return nested;
      ...
      return valueExpr.shapeExprs.reduce(function (ret, nested, iter) {
        return ret.concat(_ShExValidator._errorsMatchingValueExpr (value, nested, recurse, direct, true));
      }, []);
    } else {
   ...
   */
  this._errorsMatchingValueClass = function (value, valueExpr, recurse) {
    var errors = [];
    ...
    if (valueExpr.reference) throw Error("unexpected ref in " + JSON.stringify(valueExpr)); // !! remove reference.
    // if (negated) ;
