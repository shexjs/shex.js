  ...
  this.validate = function (db, point, shapeLabel, depth) {
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      var subVal = schema.shapes[shapeLabel].expression ?
        _toVal(schema.shapes[shapeLabel].expression, _constraintToTriples(), constraintList, neighborhood, db, depth) :
        {};
      if (subVal === null)
        continue;
 ...
 */
function _toVal (expr, constraintToTriples, constraintList, neighborhood, db, depth) { // consumes constraintToTriples
  if (expr.type === 'tripleConstraint') {
    var constraintNo = constraintList.indexOf(expr);
    var take = expr.max === undefined ? 1 : expr.max;
    var ret = {
    ...
    do {
      ...
      for (var ord = 0; ord < expr.expressions.length; ++ord) {
        ...
        var addMe = _toVal(nested, constraintToTriples, constraintList, neighborhood, db, depth);
        if (addMe === null) {
          if (expr.type === 'group')
            return null;
          else
    ...
    do {
      expr.expressions.forEach(function (nested, ord) {
        regexp += added = _toVal(nested, constraintToTriples, constraintList, neighborhood, db, depth)
        if (added === null)
          return null;
      });
    } while (added !== "");
