...
function ShExValidator(schema, options) {
  ...
  this.compileShape = function (shape) {
    ...
    return {
      regexp: shape.expression ? this._compileRegexp(shape.expression, tripleConstraints,
 ...
 */
this._compileRegexp = function (expr, tripleConstraints, predicateToTripleConstraints, optimize) {
  var _ShExValidator = this;
  var regexp = "", ret;
  ...
  if (expr.type === "TripleConstraint") {
    ...
    regexp += "((?:"+ordinal + " )" + _writeCardinality(expr.min, expr.max, expr.negated) + ")";
    ...
    expr.expressions.forEach(function (nested, ord) {
      if (ord)
        ret += "|";
      ret += _ShExValidator._compileRegexp(nested, tripleConstraints, predicateToTripleConstraints, optimize);
    });
    ...
    regexp += _ShExValidator._compileRegexp(included, tripleConstraints, predicateToTripleConstraints, optimize);
  }
  ...
  this.validate = function (db, point, shapeLabel, depth, seen) {
    ...
    var tripleList = constraintList.reduce(function (ret, constraint, ord) {
      ...
      var matchConstraints = _ShExValidator._triplesMatchingValueExpr(matchPredicate, constraint.valueExpr, constraint.inverse);
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      var subVal = expr ?
        _ShExValidator._toVal(expr, _constraintToTriples(), constraintList, neighborhood, db, recurse, depth, seen) :
        {};
      if (subVal === null)
        continue;
      if (!this.semActHandler.dispatchAll(schema.shapes[shapeLabel].semActs || [], subVal))
        continue; // some semAct aborted
      if ("semActs" in schema.shapes[shapeLabel]) { subVal.semActs = schema.shapes[shapeLabel].semActs; }
  ...
  this._triplesMatchingValueExpr = function (triples, valueExpr, inverse) {
    var _ShExValidator = this;
    var misses = [];
    var hits = [];
    triples.forEach(function (triple) {
      var value = inverse ? triple.subject : triple.object;
      var errors = _ShExValidator._errorsMatchingValueExpr(value, valueExpr, inverse, undefined);
      if (errors.length === 0) {
        hits.push(triple);
