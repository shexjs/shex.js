...
function ShExValidator(schema, options) {
  ...
  function compileShapeToAST (expression, triplePatterns) {
  ...
  this.compileShape = function (shape, triplePatterns, predicateToTriplePatterns, optimize) {
    ...
    return {
      ...
      ast: compileShapeToAST(shape.expression, []) // @@ make sure this ends up the same as _ShExValidator._triplePatternsByLabel[label]
    };
  };
   ...
   function _log () {
    ...
    while (xp.next() && ret === null) xp.
      do(function () {
        ...
        var constraintToTriples = tripleToConstraintMapping.slice().
          reduce(function (ret, c, ord) {
            if (c !== undefined)
              ret[c].push(ord);
            return ret;
          }, _seq(4).map(function () { return []; }));

        /* Walk an expression, representing each
         */
        function _compileFootprint (expr) {
          var regexp = "";

          if (expr.type === 'tripleConstraint') {
            var constraintNo = _ShExValidator._triplePatternsByLabel[shapeLabel].indexOf(expr);
            ...
            do {
              expr.expressions.forEach(function (nested, ord) {
                regexp += added = _compileFootprint(nested)
              });
            } while (added !== "");
          }
        ...
        var byConstraint =
          ...
          _compileFootprint(schema.shapes[shapeLabel].expression) : // walk schema
