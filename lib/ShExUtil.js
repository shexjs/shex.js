...
["Schema", "@context", "startActs", "start", "shapes",
 ...
 "NodeConstraint", "iri", "bnode", "nonliteral", "literal", "datatype", "length", "minlength", "maxlength", "pattern", "mininclusive", "minexclusive", "maxinclusive", "maxexclusive", "totaldigits", "fractiondigits", "values",
 "ShapeNot", "shapeExpr",
...
var ShExUtil = {
    ...
    var r = {
      ...
      visitTripleConstraint: function (expr) {
        return this._maybeSet(expr, { type: "TripleConstraint" }, "TripleConstraint",
                              ["productionLabel", "inverse", "predicate", "valueExpr",
      ...
      visitValues: function (values) {
        ...
        return values.map(function (t) {
          return typeof t === "object" && !("value" in t) ? _Visitor.visitStemRange(t) : t;
        });
      },
  ...
  valToValues: function (val) {
    ...
    if (val.type === "NodeTest") {
      ...
      return val.solutions.reduce((ret, sln) => {
        Object.assign(ret, _ShExUtil.valToValues(sln));
        return ret;
      }, {});
      ...
      if ("solutions" in val) {
        ...
        val.solutions.forEach(sln => {
          ...
          if (chaseList(sln.referenced, toAdd)) {
            ...
            var newElt = { value: sln.object };
            if ("referenced" in sln) {
              var t = _ShExUtil.valToValues(sln.referenced);
              if (t)
                newElt.nested = t;
            }
            vals.push(newElt);
          ...
          function chaseList (li) {
            ...
            if ("solution" in li && "solutions" in li.solution &&
              ...
              var newElt = { value: member.object };
              if ("referenced" in member) {
                var t = _ShExUtil.valToValues(member.referenced);
                if (t)
                  newElt.nested = t;
              }
              vals.push(newElt);
  ...
  valuesToSchema: function (values) {
    ...
    if (t === SX.Schema) {
      ...
      if (SX.startActs in v)
        ret.startActs = v[SX.startActs].map(e => {
          var ret = {
            type: "SemAct",
            name: e.nested[SX.name][0].value
          };
          if (SX.code in e.nested)
            ret.code = e.nested[SX.code][0].value.value;
          return ret;
        });
      if (values[SX.start])
        ret.start = shapeExpression(values[SX.start][0].nested);
      var shapes = values[SX.shapes];
      if (shapes) {
        ret.shapes = shapes.map(v => {
          var ret = shapeExpression(v.nested);
          ret.label = v.value;
          return ret;
        });
      }
    ...
    function shapeExpression (v) {
      // shapeExpr = ShapeOr | ShapeAnd | ShapeNot | NodeConstraint | Shape | ShapeRef | ShapeExternal;
      var t = v[RDF.type][0].value;
      if (t === SX.ShapeAnd) {
        return {
          type: "ShapeAnd",
          shapeExprs: v[SX.shapeExprs].map(e => {
            return shapeExpression(e.nested);
          })
        };
      } else if (t === SX.ShapeOr) {
        return {
          type: "ShapeOr",
          shapeExprs: v[SX.shapeExprs].map(e => {
            return shapeExpression(e.nested);
      ...
      } else if (t === SX.ShapeNot) {
        return {
          type: "ShapeNot",
          shapeExpr: shapeExpression(v[SX.shapeExpr][0].nested)
        };
      } else if (t === SX.Shape) {
        var ret = { type: "Shape" };
        ["closed"].forEach(a => {
          if (SX[a] in v)
            ret[a] = !!v[SX[a]][0].value.value;
        });
        if (SX.extra in v)
          ret.extra = v[SX.extra].map(e => { return e.value; });
        if (SX.expression in v)
          ret.expression = tripleExpression(v[SX.expression][0].nested);
        if (SX.annotation in v)
          ret.annotations = v[SX.annotation].map(e => {
            return {
              type: "Annotation",
              predicate: e.nested[SX.predicate][0].value,
              object: e.nested[SX.object][0].value
            };
          });
        if (SX.semActs in v)
        ...
        return ret;
      } else if (t === SX.ShapeRef) {
        return {
          type: "ShapeRef",
          reference: v[SX.reference][0].value
        };
      } else if (t === SX.ShapeExternal) {
        return {
          type: "ShapeExternal"
        };
        ...
        if (SX.nodeKind in v)
          ret.nodeKind = v[SX.nodeKind][0].value.substr(SX._namespace.length);
        ["length", "minlength", "maxlength", "mininclusive", "maxinclusive", "minexclusive", "maxexclusive", "totaldigits", "fractiondigits"].forEach(a => {
          if (SX[a] in v)
            ret[a] = parseFloat(v[SX[a]][0].value.value);
        });
        if (SX.pattern in v)
          ret.pattern = v[SX.pattern][0].value.value;
        if (SX.datatype in v)
          ret.datatype = v[SX.datatype][0].value;
        return ret;
      } else {
        throw Error("unknown tripleExpression type in " + JSON.stringify(v));
      }
    ...
    function objectValue (v) {
      if ("nested" in v) {
        var t = v.nested[RDF.type][0].value;
        if (t === SX.Stem) {
          return {
            type: "Stem",
            stem: v.nested[SX.stem][0].value.value
          };
        } else if (t === SX.StemRange) {
          var st = v.nested[SX.stem][0];
          var stem = st;
          if (typeof st === "object") {
            if (typeof st.value === "object") {
              stem = st.value.value;
            } else if (st.value.startsWith("_:")) {
              stem = { type: "Wildcard" };
            }
          }
          var ret = {
    ...
    function tripleExpression (v) {
      ...
      if (t === SX.TripleConstraint) {
        ...
        if (SX.valueExpr in v)
          ret.valueExpr = shapeExpression(v[SX.valueExpr][0].nested);
        minMaxAnnotSemActs(v, ret);
        return ret;
      } else if (t === SX.EachOf) {
        return Object.assign({
          type: "EachOf",
          expressions: v[SX.expressions].map(e => {
            return tripleExpression(e.nested);
          })
        }, minMaxAnnotSemActs(v, {}));
      } else if (t === SX.OneOf) {
        return Object.assign({
          type: "OneOf",
          expressions: v[SX.expressions].map(e => {
      ...
      } else if (t === SX.Inclusion) {
        return {
          type: "Inclusion",
          include: v[SX.include][0].value
        };
      } else {
        throw Error("unknown tripleExpression type in " + JSON.stringify(v));
      }
    ...
    function minMaxAnnotSemActs (v, ret) {
      if (SX.min in v)
        ret.min = parseInt(v[SX.min][0].value.value);
      if (SX.max in v) {
        ret.max = parseInt(v[SX.max][0].value.value);
        if (isNaN(ret.max))
          ret.max = "*";
      }
      if (SX.annotation in v)
