...
var ShExUtil = {
    ...
    v.visitExpression = function (expr) {
      if ("id" in expr) {
        if (expr.id in knownExpressions)
          return expr.id;
        knownExpressions[expr.id] = expr;
        delete expr.id;
      }
      return oldVisitExpression.call(this, expr);
    }
  ...
  ShExRtoShExJ: function (schema) {
    var _ShExUtil = this;
    // if ("start" in schema) {
    //   var v = _ShExUtil.ShExRVisitor(newProductions);
    //   schema.start = v.visitShapeExpr(schema.start);
    // }
    if ("shapes" in schema) {
      var knownShapeExprs = schema.shapes.map(sh => { return sh.id; });
      var v = _ShExUtil.ShExRVisitor(knownShapeExprs);
      schema.shapes = schema.shapes.map(sh => {
        // var id = sh.id;
        // delete sh.id;
        var ret = v.keepShapeExpr(sh);
        // ret.id = id;
        return ret;
      });
    }
  ...
  canonicalize: function (schema) {
    delete schema.prefixes;
    delete schema.base;
    // Don't delete schema.productions as it's part of the AS.
    ...
    function tripleExpr (v) {
      ...
      if (t === SX.TripleConstraint) {
        ...
        if (SX.valueExpr in v)
          ret.valueExpr = extend({id: v[SX.valueExpr][0].ldterm}, shapeExpr(v[SX.valueExpr][0].nested));
        minMaxAnnotSemActs(v, ret);
        return ret;
      } else {
