...
var ShExUtil = {
  ...
  valuesToSchema: function (values) {
    ...
    function objectValue (v, langtag) {
      if ("nested" in v) {
        ...
        if ([SX.IriStem, SX.LiteralStem, SX.LanguageStem].indexOf(t) !== -1) {
          ...
          if (ldterm.type === "http://www.w3.org/2001/XMLSchema#anyUri" || langtag) // !!! json-ld issue
            ldterm = ldterm.value;
          return {
          ...
          var ret = {
            type: t.substr(SX._namespace.length),
            stem: ["LanguageStemRange", "LiteralStemRange"].indexOf(t.substr(SX._namespace.length)) !== -1 && stem.type !== "Wildcard" ? stem.value : stem
          };
          function foo (x) {
            return x.ldterm.value;
          }
          if (SX.exclusion in v.nested)
            // {"ldterm":"_:b5","nested":{
            //   rdf:type:[{ldterm:sx:IriStem}],
            //   sx:stem:[{"ldterm":{"value":"http://a.example/v1","type":xsd:anyUri}}]}}
            ret.exclusions = v.nested[SX.exclusion].map(v1 => {
              return ret.type === "LanguageStemRange" || ret.type === "LiteralStemRange" ? objectValue(v1, true) : objectValue(v1);
            });
          return ret;
        } else {
        ...
        return langtag ? v.ldterm.value : v.ldterm;
      }
    }
