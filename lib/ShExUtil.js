  ...
  Visitor: function () {
    ...
    var r = {
      ...
      visitNodeConstraint: function (shape, label) {
        ...
        this._maybeSet(shape, ret, "NodeConstraint",
                       [
                         ...
                         "closed", "nodeKind", "datatype", "pattern", "length",
      ...
      visitValueExpr: function (expr) {
        var r = expr.type === "NodeConstraint" ? this.visitValueClass(expr) :
    ...
    Object.keys(schema.shapes || []).forEach(function (label) {
      function _walkShapeExpression (shapeExpr, negated) {
        if (shapeExpr.type === "ShapeOr" || shapeExpr.type === "ShapeAnd") {
          ...
          _walkNodeConstraint(shapeExpr, negated ^ !!shapeExpr.negated);
        } else if (shapeExpr.type === "ShapeRef") {
          ret.add(label, shapeExpr.reference);
        } else
          throw Error("expected Shape{And,Or,Ref} or NodeConstraint in " + shapeExpr);
      }
      
      function _walkNodeConstraint (shape, negated) {
