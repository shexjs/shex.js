...
var ShExUtil = {
    ...
    var r = {
      ...
      visitShapeExpr: function (expr, label) {
        var r =
            ...
            expr.type === "ShapeRef" ? this.visitValueRef(expr) :
            null;
        if (r === null)
      ...
      visitShapeNot: function (expr, label) {
        return { type: expr.type, shape: this.visitShapeExpr(expr.shape, label) };
      },
      ...
      visitReference: function (ref) {
        return { type: "ShapeRef", reference: ref };
      },
      ...
      visitValueExpr: function (expr) {
        var r =
            expr.type === "Shape" ? this.visitValueShape(expr) :
            expr.type === "NodeConstraint" ? this.visitValueClass(expr) :
            expr.type === "ShapeAnd" ? this.visitValueAnd(expr) :
            expr.type === "ShapeOr" ? this.visitValueOr(expr) :
            expr.type === "ShapeNot" ? this.visitValueNot(expr) :
            expr.type === "ShapeRef" ? this.visitReference(expr.reference) :
            null;
        if (r === null)
      ...
      visitValueShape: function (valueClass) {
        return this._maybeSet(valueClass, { type: "Shape" }, "NodeConstraint",
                              ["expression", "closed", "extra", "semActs"]);
      },

      visitValueClass: function (valueClass) {
        return this._maybeSet(valueClass, { type: "NodeConstraint" }, "NodeConstraint",
    ...
    Object.keys(schema.shapes || []).forEach(function (label) {
      function _walkShapeExpression (shapeExpr, negated) {
        if (shapeExpr.type === "ShapeOr" || shapeExpr.type === "ShapeAnd") {
          ...
          _walkShapeExpression(shapeExpr.shape, negated ^ 1); // !!! test negation
  ...
  validateSchema: function (schema) { // obselete, but may need other validations in the future.
    ...
    var oldVisitReference = visitor.visitReference;
    visitor.visitReference = function (reference) {
      if (!(reference in schema.shapes))
        throw Error("Structural error: reference to " + reference + " not resolvable.");
      return oldVisitReference.call(visitor, reference);
    }
    var oldVisitValueRef = visitor.visitValueRef;
    visitor.visitValueRef = function (shapeRef) {
      if (!(shapeRef.reference in schema.shapes))
        throw Error("Structural error: reference " + JSON.stringify(shapeRef) + " not resolvable.");
      if (shapeRef.reference === currentShape)
        throw Error("Structural error: circular reference to " + currentShape + ".");
      return oldVisitValueRef.call(visitor, shapeRef);
    }
