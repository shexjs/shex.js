    ...
    function objectValue (v, langtag) {
      if ("nested" in v) {
        ...
        if ([SX.IriStem, SX.LiteralStem, SX.LanguageStem].indexOf(t) !== -1) {
          ...
          var ret = {
            type: t.substr(SX._namespace.length),
            stem: t.substr(SX._namespace.length) === "LanguageStemRange" && stem.type !== "Wildcard" ? stem.value : stem
          };
          function foo (x) {
          ...
          if (SX.exclusion in v.nested)
            ...
            ret.exclusions = v.nested[SX.exclusion].map(v1 => {
              return ret.type === "LanguageStemRange" ? objectValue(v1, true) : objectValue(v1);
            });
          return ret;
        } else {
