...
var ShExUtil = {
  ...
  Visitor: function () {
    ...
    var r = {
      ...
      visitValueClasses: function (valueClasses) {
        ...
        return valueClasses === undefined ?
          ...
          visitMap(valueClasses, function (val) {
            return _Visitor.visitValueClassDefn(val); });
      },
      visitValueClassDefn: function (valueClassDefn, label) {
        var ret = { type: "valueClassDefn" };
        _ShExUtil._expect(valueClassDefn, 'type', 'valueClassDefn');

        this._maybeSet(valueClassDefn, ret, "valueClassDefn",
                       ["value"]);
        return ret;
      },
      ...
      // _visitValueGroup: visit a grouping expression (vcand)
      _visitValueGroup: function (expr, type) {
        var _Visitor = this;
        var r = { type: expr.type };
        r.valueClasses = expr.valueClasses.map(function (nested) { // !!! valueExprs
          return _Visitor.visitValue(nested); // !!! visitValueExpr
        });
        return r;
      },

      visitValue: function (expr) { // !!! visitValueExpr
        var r = expr.type === 'valueClass' ? this.visitValueClass(expr) :
          expr.type === 'vcand' ? this.visitVcand(expr) :
          expr.type === 'vcref' ? this.visitVcref(expr) :
          null;
        if (r === null)
          throw Error("unexpected value type: " + expr.type); // !!! valueExpr
        else
          return r;
      },
      ...
      _maybeSet: function (obj, ret, context, members) {
        var _Visitor = this;
        var unknownMembers = Object.keys(obj).reduce(function (ret, k) {
          return k !== "type" && members.indexOf(k) === -1 ? ret.concat(k) : ret;
        }, []);
    ...
    r.visitVcand = r._visitValueGroup;
    return r;
  },
      ...
      function _walkExpression (expr, indent, precedent) {
        ...
        if (expr.type === 'tripleConstraint') {
          function _checkValueExpr (v) {
            ...
            } else if (v.type === "vcref") {
              _checkValueExpr(schema.valueClasses[v.valueClassRef].value);
            } else {
              expect(v, 'type', 'vcand');
              // !!! finish recursion
            }
          }
          _checkValueExpr(expr.value);
        }
