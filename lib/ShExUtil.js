...
var ShExUtil = {
  ...
  valuesToSchema: function (values) {
    ...
    if (t === SX.Schema) {
      ...
      if (values[SX.start])
        ret.start = shapeExpr(values[SX.start][0].nested);
      var shapes = values[SX.shapes];
      if (shapes) {
        ret.shapes = shapes.map(v => {
          var ret = shapeExpr(v.nested);
          ret.label = v.value;
          return ret;
        });
      }
    ...
    function foo (v, elts, f) {
      var t = v[RDF.type][0].value.substr(SX._namespace.length);
      var elt = elts[t];
      if (!elt)
        return Missed;
      if (elt.nary) {
        ...
        if (elt.prop) {
          ret[elt.prop] = valuate(v[SX[elt.prop]][0]);
        }
        return ret;
      }

      function valuate (x) {
        return elt.expr ? f(x.nested) : x.value;
      }
    }
    function shapeExpr (v) {
      ...
      if (t === SX.Shape) {
        ...
        if (SX.expression in v)
          ret.expression = tripleExpr(v[SX.expression][0].nested);
    ...
    function tripleExpr (v) {
      ...
      if (t === SX.TripleConstraint) {
        ...
        if (SX.valueExpr in v)
          ret.valueExpr = shapeExpr(v[SX.valueExpr][0].nested);
        minMaxAnnotSemActs(v, ret);
        return ret;
      } else {
