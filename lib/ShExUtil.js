...
var ShExUtil = {
  ...
  Visitor: function () {
      ...
      visitNodeConstraint: function (shape, label) {
        ...
        this._maybeSet(shape, ret, "NodeConstraint",
                       [ "id",
                         ...
                         "totaldigits", "fractiondigits", "values", "annotations"]);
        return ret;
      },
    ...
    r.visitExtends = r.visitRestricts = r.visitExtra = r.visitAnnotations = r._visitList;
  ...
  nestShapes: function (schema, options = {}) {
    ...
    schema.shapes.forEach(shape => {
      shapeHash[shape.id] = shape
      noteReference(shape.id, null) // just note the shape so we have a complete list at the end
      ...
      if (shape.type === 'Shape') {
        ...
        if ('expression' in shape) {
          (_ShExUtil.simpleTripleConstraints(shape) || []).forEach(tc => {
            noteReference(_ShExUtil.getValueType(tc.valueExpr, true), tc)
          })
        }
    ...
    let nestables = Object.keys(shapeReferences).filter(
      label => shapeReferences[label].length === 1
        && shapeReferences[label][0].type !== 'Shape' // no inheritance support yet
        && _ShExUtil.skipDecl(shapeHash[label]).type !== 'Shape' // Don't nest e.g. valuesets for now
    )
    if (!options.no) {
      if (!('transform' in options)) {
        options.transform = (function () {
          let map = Object.keys(shapeHash).reduce((acc, k, idx) => {
      ...
      nestables.forEach(label => {
        let shapeExpr = shapeHash[label]
        if (shapeExpr.id !== label) { throw Error('assertion: ' + shapeExir.id + ' !== ' + label) }
        let newName = options.transform(label, shapeExpr)
        shapeExpr.id = newName
        if (shapeReferences[label].length !== 1) { throw Error('assertion: ' + label + ' doesn\'t have one reference: [' + shapeReferences[label] + ']') }
        let ref = shapeReferences[label][0]
        if (ref.type === 'TripleConstraint') {
          if (typeof ref.valueExpr === 'string') {
            ref.valueExpr = newName
          } else {
            throw Error('assertion: rename not implemented for TripleConstraint expr: ' + ref.valueExpr)
   ...
   *
   */
  getPredicateUsage: function (schema, untyped = {}, anyURIasDataProperty = false) {
    var _ShExUtil = this;
    ...
    Object.keys(schema.shapes).forEach(shapeLabel => {
      ...
      if (shapeExpr.type === 'Shape') {
        ...
        tcs.forEach(tc => {
          let newType = _ShExUtil.getValueType(tc.valueExpr, anyURIasDataProperty)
  ...
  getValueType: function (valueExpr, anyURIasDataProperty) {
    if (typeof valueExpr === 'string') { return valueExpr }
    if (valueExpr.reference) { return valueExpr.reference }
    let dt = valueExpr.datatype
    if (dt) {
      if (dt === XSD.anyURI && !anyURIasDataProperty) {
        return OWL.Thing
      }
      return dt
    }
    throw Error('no value type for ' + JSON.stringify(valueExpr))
  },
