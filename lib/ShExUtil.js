    ...
    var r = {
      ...
      visitTripleConstraint: function (expr) {
        return this._maybeSet(expr, { type: "tripleConstraint" }, "tripleConstraint",
                              ["inverse", "negated", "predicate", "valueClassRef", "valueExpr",
      ...
      _visitValueGroup: function (expr, type) {
        var _Visitor = this;
        var r = { type: expr.type };
        r.valueClasses = expr.valueClasses.map(function (nested) {
        ...
        if (expr.type === 'tripleConstraint') {
          function _checkValueExpr (v) {
            if (v.type === "valueClass") {
              ...
              _checkValueExpr(schema.valueClasses[v.valueClassRef].valueExpr);
            } else {
              expect(v, 'type', 'vcand');
              // !!! finish recursion
            }
