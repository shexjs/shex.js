...
var ShExUtil = {
    ...
    function objectValue (v) {
      if ("nested" in v) {
        ...
        if (t === SX.Stem) {
          return {
            type: "Stem",
            stem: v.nested[SX.stem][0].ldterm.value
          };
  ...
  valToSimple: function (val) {
    var _ShExUtil = this;
    if (val.type === "SolutionList") {
      var reduced = val.solutions.reduce((ret, sln) => {
        var r = _ShExUtil.valToSimple(sln);debugger;
        return r.reduce((ret, pair) => {
          if (!(pair.node in ret)) {
            ret[pair.node] = [pair.shape];
          } else if (ret[pair.node].indexOf(pair.shape) === -1)
            ret[pair.node].push(pair.shape);
          return ret
        }, ret);
      }, {});
      var ret = [];
      ...
      return [{node:val.node, shape:val.shape}].concat(_ShExUtil.valToSimple(val.solution));
    } else if (val.type === "EachOfSolutions" || val.type === "OneOfSolutions") {
      return val.solutions.reduce((outer, sln) => {
        return sln.expressions.reduce((inner, exp) => {
          var n = _ShExUtil.valToSimple(exp);
          return inner.concat.apply(inner, n);
        }, outer);
      }, []);
    } else if (val.type === "OneOfSolutions") {
      return val.solutions.reduce((ret, sln) => {
        Object.assign(ret, _ShExUtil.valToSimple(sln));
        return ret;
      }, {});
    } else if (val.type === "TripleConstraintSolutions") {
      if ("solutions" in val) {
        return val.solutions.reduce((ret, sln) => {
          ...
          var toAdd = [];
          if (chaseList(sln.referenced, toAdd)) {
            return ret.concat(toAdd);
          } else {
            return ret.concat(_ShExUtil.valToSimple(sln.referenced));
          }
          function chaseList (li) {
            if (!li) return false;
            if (li.node === RDF.nil) return true;
  ...
  errsToSimple: function (val, node, shape) {
    ...
    if (val.type === "FailureList") {
      ...
      return ["validating " + val.node + " as " + val.shape + ":"].concat(val.errors.reduce((ret, e) => {
        return ret.concat(_ShExUtil.errsToSimple(e).map(x => { return "  " + x; }));
      }, []));
    } else if (val.type === "TypeMismatch") {
      return val.errors.reduce((ret, e) => {
        return typeof e === "string" ? [e] : _ShExUtil.errsToSimple(e);
      }, []);
    ...
    } else if (val.type === "MissingProperty") {debugger;
