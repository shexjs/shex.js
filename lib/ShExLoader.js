...
function LoadPromise (shex, json, turtle, jsonld, schemaOptions, dataOptions) {
  ...
  function add (src, metaList, mediaType, parserWrapper, target, options) {
    return loadList(src, mediaType, function (text, url) {
      var meta = {mediaType: mediaType, url: url, base: url, prefixes: {}};
      metaList.push(meta);
      return new Promise(function (resolve, reject) {
        parserWrapper(resolve, reject, text, mediaType, url, target, meta, options);
      });
    })
  }
...
function loadShExImports (from, parser, transform) {
  var seen = [];
  var ret = { type: "Schema" };
  return getAll(from).then(function () {
    ShExUtil.isWellDefined(ret);
    return ret;
  });
  function getAll (i) {
    seen.push(i);
    return GET(i).then(function (loaded) {
...
return { load: LoadPromise, loadExtensions: LoadExtensions, GET: GET, loadShExImports: loadShExImports };
})();
