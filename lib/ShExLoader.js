...
function loadShExSync (filename, shexParser, transform) {
  var toLoad = [filename];
  var loaded = [];
  var ret = { type: "Schema" };
  while (toLoad.length) {
    var f = toLoad.pop();
    loaded.push(f);
    var shexc = FS.readFileSync(f, "utf8");
    var schema = shexParser.parse(shexc);
    ShExUtil.merge(ret, schema, false, true);
    if ("imports" in schema) {
      toLoad = toLoad.concat(schema.imports.
                             map(function (i) {
                               return transform(i);
                             }).
                             filter(function (i) {
                               return loaded.indexOf(i) === -1;
                             }));
    }
  };
...
return { load: LoadPromise, loadExtensions: LoadExtensions, GET: GET, loadShExSync: loadShExSync };
})();
