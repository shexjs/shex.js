...
function LoadPromise (shex, json, turtle, jsonld) {
  ...
  return Promise.all([].
                     ...
                     concat(add(shex, returns.schemaMeta,
                                "text/shex", parseShExC, returns.schema)).
                     concat(add(json, returns.schemaMeta,
                                "text/json", parseShExJ, returns.schema)).
                     concat(add(turtle, returns.dataMeta,
                                "text.turtle", parseTurtle, returns.data)).
                     concat(add(jsonld, returns.dataMeta,
                                "application/ld+json", parseJSONLD, returns.data))
                    ).then(function () { return returns; });
}

function parseShExC (resolve, reject, text, mediaType, url, schema, meta) {
  var s;
  try {
    s = (new ShExParser(url)).parse(text);
  } catch (e) {
    var t = Error("error parsing ShEx " + url + ": " + (e.message || e));
    t.stack = e.stack;
    reject(t);
  }
  if (s) {
...
function parseShExJ (resolve, reject, text, mediaType, url, schema, meta) {
  try {
...
function parseTurtle (resolve, reject, text, mediaType, url, data, meta) {
...
function parseJSONLD (resolve, reject, text, mediaType, url, data, meta) {
