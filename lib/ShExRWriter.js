...
ShExRWriter.prototype = {
  ...
  _writeShapeExpr: function (shapeExpr, done, lead) {
    ...
    // !!! []s for precedence!
      ...
      pieces.push(_ShExRWriter._writeShapeExpr(shapeExpr.shapeExpr, done, lead));
  ...
  _writeShape: function (shape, done, lead) {
    ...
    try {
      ...
      [{keyword: "extends", marker: "&"}].forEach(pair => {
         // pieces = pieces.concat(_ShExRWriter._writeShapeExpr(expr.valueExpr, done, true, 0));
         if (shape[pair.keyword] && shape[pair.keyword].length > 0) {
           shape[pair.keyword].forEach(function (i, ord) {
             if (ord)
               pieces.push(" ")
             pieces.push(pair.marker);
             pieces = pieces.concat(_ShExRWriter._writeShapeExpr(i, done, lead+TAB));
           });
           pieces.push(" ");
         }
       });
