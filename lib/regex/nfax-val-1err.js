...
function compileNFA (schema, shape) {
    ...
    function rbenx_match (graph, node, constraintList, constraintToTripleMapping, tripleToConstraintMapping, neighborhood, recurse, semActHandler, checkValueExpr, trace) {
      var _this = this;
      ...
      if (_this.states.length === 1)
        return matchedToResult([], constraintList, neighborhood, recurse, semActHandler, checkValueExpr);
	  ...
	  if ("reference" in valueExpr) {
	    ...
	    if (N3Util.isBlank(ref))
          ...
          return {
            type: state.c.negated ? "NegatedProperty" : "MissingProperty",
      ...
      return "errors" in chosen.matched ?
        chosen.matched :
        matchedToResult(chosen.matched, constraintList, neighborhood, recurse, semActHandler, checkValueExpr);
    }

    function matchedToResult (matched, constraintList, neighborhood, recurse, semActHandler, checkValueExpr) {
      var last = [];
      var errors = [];
      var skips = [];
      var ret = matched.reduce((out, m) => {
        ...
        ptr.solutions = m.triples.map(tno => {
          ...
          errors = errors.concat(checkValueExpr(ptr.inverse ? triple.subject : triple.object, ptr.valueExpr, ptr.inverse, function (focus, shape) {
...
var exports = { name: "nfax-val-1err", compile: compileNFA };
