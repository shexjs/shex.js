        ...
        for (var threadno = 0; threadno < clist.length; ++threadno) {
          ...
          function resolveAccessor (matched, accessor) {
            return matched.reduce((ret, m) => {
              return m.c.productionLabel === accessor.productionLabel ? ret.concat(m.triples.map(t => {
                var triple = neighborhood[t];
                var node = m.inverse ? triple.subject : triple.object;
                switch (accessor.type) {
                case "TermAccessor":
                  return N3Util.isLiteral(node) ? N3Util.getLiteralValue(node) :
                    N3Util.isBlank(node) ? node.substr(2) :
                    node ;
                case "LangtagAccessor":
                  return N3Util.isLiteral(node) ? N3Util.getLiteralLanguage(node) : "";
                case "DatatypeAccessor":
                  return N3Util.isLiteral(node) ? N3Util.getLiteralType(node) : "";
                default:
