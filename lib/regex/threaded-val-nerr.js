...
function vpEngine (schema, shape) {
    ...
    function match (graph, node, constraintList, constraintToTripleMapping, tripleToConstraintMapping, neighborhood, recurse, direct, semActHandler, checkValueExpr, trace) {
            ...
            do {
              ret.push({
                ...
                expression: extend({
                  ...
                  //   }
                  // })
                }, "valueExpr" in expr ? { valueExpr: expr.valueExpr } : {}, minmax)
              });
            } while ((function () {
      ...
      return longerChosen !== null ? finish(longerChosen.expression, constraintList, neighborhood, recurse, direct, semActHandler, checkValueExpr) :
        ret.length > 1 ? {
          type: "PossibleErrors",
