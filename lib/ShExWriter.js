...
function _dumpValueExpr (_ShExWriter, valueExpr, done) {
  ...
  // !!! []s for precedence!
    ...
    valueExpr.shapeExprs.forEach(function (expr, ord) {
      if (ord > 0 && !(valueExpr.shapeExprs[ord-1].type === "ShapeRef" && expr.type === "Shape")) // !!! grammar rules too weird here
        pieces.push(" AND ");
    ...
    pieces.push(dumpValueClass(_ShExWriter, valueExpr, done));
  return pieces.join('');
}

function dumpValueClass (_ShExWriter, v, done) {
  var pieces = [];
...
ShExWriter.prototype = {
  ...
  _writeSchema: function (schema, done) {
    ...
    if (schema.valueExprDefns) {
      Object.keys(schema.valueExprDefns).forEach(function (label) {
        _ShExWriter._write("$"+_ShExWriter._encodeShapeName(label)+" = "+_dumpValueExpr(_ShExWriter, schema.valueExprDefns[label].valueExpr, done)+"\n");
      });
    }
    if (schema.start)
  ...
  _writeShape: function (shape, done, forceBraces) {
    ...
    try {
      ...
      ['length', 'minlength', 'maxlength' // !!! not complete
      ].forEach(function (a) {
        if (shape[a])
