...
function _dumpValueExpr (_ShExWriter, valueExpr, done) {
  var pieces = [];
  if (valueExpr.type === "ShapeRef")
    pieces.push("@", _ShExWriter._encodeShapeName(valueExpr.reference));
  // !!! ()s for precedence!
  else if (valueExpr.type === "ShapeAnd") {
    valueExpr.shapeExprs.forEach(function (expr, ord) {
      if (ord > 0 && !(valueExpr.shapeExprs[ord-1].type === "ShapeRef" && expr.type === "NodeConstraint")) // !!! grammar rules too weird here
        pieces.push(" AND ");
      pieces.push(_dumpValueExpr(_ShExWriter, expr, done));
    });
    ...
    pieces.push("NOT ");
    pieces.push(_dumpValueExpr(_ShExWriter, valueExpr.shape, done));
  } else if (valueExpr.type === "Shape") {
    // pieces = pieces.concat(_ShExWriter._writeShape(valueExpr, done, forceBraces));
    pieces.push(dumpValueShape(_ShExWriter, valueExpr, done));
  } else if (valueExpr.type === "NodeConstraint") {
    // pieces = pieces.concat(_ShExWriter._writeNodeConstraint(valueExpr, done, forceBraces));
    pieces.push(dumpValueNodeConstraint(_ShExWriter, valueExpr, done));
  } else
    throw Error("expected Shape{,And,Or,Ref} or NodeConstraint in " + util.inspect(valueExpr));
  return pieces.join('');
}

function dumpValueShape (_ShExWriter, v, done) {
  var pieces = [];
  _ShExWriter._expect(v, "type", "Shape");
  if (!v.reference && !v.nodeKind && !v.values && !v.datatype && !v.expression && !v.minlength && !v.maxlength)
    pieces.push(". "); // !! not needed
  else {
    if (v.expression || v.values) {
      pieces = pieces.concat(_ShExWriter._writeShapeExpr(v, done, false, 0).map(s => { return "  " + s; }));
    }
  }
...
function dumpValueNodeConstraint (_ShExWriter, v, done) {
  _ShExWriter._expect(v, "type", "NodeConstraint");
  if (!v.nodeKind && !v.values && !v.datatype && !v.minlength && !v.maxlength)
    pieces.push(". "); // !! not needed
  else {
    var pieces = [];
    ...
    _ShExWriter._fillNodeConstraint(pieces, v, done);
    return pieces.join('');
  ...
  _writeShapeExpr: function (shapeExpr, done, forceBraces, parentPrec) {
    ...
    // !!! []s for precedence!
      ...
      shapeExpr.shapeExprs.forEach(function (expr, ord) {
        if (ord > 0)
          pieces.push(
            (shapeExpr.shapeExprs[ord-1].type === "NodeConstraint" && (shapeExpr.shapeExprs[ord].type === "Shape" || shapeExpr.shapeExprs[ord].type === "ShapeRef")) ?
              " " :
              " AND ");
      ...
      pieces.push("NOT ");
      pieces = pieces.concat(_ShExWriter._writeShapeExpr(shapeExpr.shape, done, forceBraces, 1));
      ...
      function _writeExpression (expr, indent, parentPrecedence) {
        ...
        if (expr.type === "TripleConstraint") {
          ...
          if ("valueExpr" in expr)
            pieces.push(_dumpValueExpr(_ShExWriter, expr.valueExpr, done));
          else
            pieces.push(". ");
