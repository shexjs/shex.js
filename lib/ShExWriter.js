...
function _dumpValueExpr (_ShExWriter, valueExpr) {
  var pieces = [];
  if (valueExpr.type === "vcref")
    pieces.push("$", _ShExWriter._encodeShapeName(valueExpr.valueClassRef));
  else if (valueExpr.type === "vcand") {
    valueExpr.valueClasses.forEach(function (expr, ord) {
      if (ord > 0)
        pieces.push(" AND ");
  ...
  _writeSchema: function (schema, done) {
    ...
    if (schema.valueClasses) {
      Object.keys(schema.valueClasses).forEach(function (label) {
        _ShExWriter._write("$"+_ShExWriter._encodeShapeName(label)+" = "+_dumpValueExpr(_ShExWriter, schema.valueClasses[label].value)+"\n");
      });
    }
    if (schema.startActs)
  ...
  _writeShape: function (shape, label, done) {
	...
	function _writeExpressionActions (semActs) {
	    ...
	    semActs.forEach(function (act) {
	      ...
	      pieces.push(_ShExWriter._encodeValue(act.name));
              pieces.push("{"+act.code.replace(/\\/g, "\\\\").replace(/%/g, "\\%")+"%"+"}");
	    });
          }
        }
	...
	if (expr.type === 'tripleConstraint') {
	  ...
	  pieces.push(indent,
          ...
          pieces.push(_dumpValueExpr(_ShExWriter, expr.value));
