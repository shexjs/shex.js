...
ShExWriter.prototype = {
  ...
  _writeShape: function (shape, done, forceBraces) {
    ...
    try {
      ...
      function _writeCardinality (min, max) {
        ...
        else if (min === 0 && max === "INF") pieces.push("*");
        else if (min === undefined && max === undefined)                         ;
        else if (min === 1 && max === "INF") pieces.push("+");
        else
          pieces.push("{", min, ",", (max === "INF" ? "*" : max), "}"); // by coincidence, both use the same character.
      }
  ...
  _fillNodeConstraint: function (pieces, v, done) {
    ...
    if ('pattern' in v) {
      pieces.push("~", _ShExWriter._encodeValue({ value: v.pattern }), " ");
    }
