  ...
  _writeShapeExpr: function (shapeExpr, done, forceBraces, parentPrec) {
    ...
    // !!! []s for precedence!
      ...
      shapeExpr.shapeExprs.forEach(function (expr, ord) {
        if (ord > 0) {
	  var prv = ("reference" in shapeExpr.shapeExprs[ord-1]);
	  var cur = ("reference" in shapeExpr.shapeExprs[ord  ]);
          pieces.push(prv^cur ? " " : " AND ");
	}
      ...
      function _writeExpression (expr, indent, parentPrecedence) {
        ...
        if (expr.type === "TripleConstraint") {
          ...
          if ("valueExpr" in expr)
            pieces = pieces.concat(_ShExWriter._writeShapeExpr(expr.valueExpr, done, false, 0));
          else
            pieces.push(". ");
