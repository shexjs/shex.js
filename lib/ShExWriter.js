...
ShExWriter.prototype = {
  ...
  _writeSchema: function (schema, done) {
    ...
    if (schema.valueClasses) {
      Object.keys(schema.valueClasses).forEach(function (label) {
        _ShExWriter._write("$"+_ShExWriter._encodeShapeName(label)+" = "+dumpValueClass(_ShExWriter, schema.valueClasses[label])+"\n");
      });
    }
    if (schema.startAct)
  ...
  _writeShape: function (shape, label, done) {
	...
	if (expr.type === 'tripleConstraint') {
	  ...
	  pieces.push(indent,
		      ...
		      " ");
          if (expr.valueClassRef)
            pieces.push("$", _ShExWriter._encodeShapeName(expr.valueClassRef));
          else
            pieces.push(dumpValueClass(_ShExWriter, expr.valueClasses ? expr.valueClasses[expr.valueClassRef] : expr.value));
	  ...
	  _semanticActions(expr.semAct);
          ...
          var needsParens = "min" in expr || "max" in expr || "annotations" in expr || "semAct" in expr;
	  ...
	  _semanticActions(expr.semAct);
          ...
          pieces.push("(");
          _writeExpression(expr.expression, indent+"  ", 0)
          pieces.push(")");
	  _writeCardinality(expr.min, expr.max); // t: open1dotclosecardOpt
          _annotations(expr.annotations);
	  _semanticActions(expr.semAct);
