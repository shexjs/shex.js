...
non-abstract shape extending Resource", which is what the union was for. If
the enumeration is wanted for other reasons, it needs a different name
(`<AnyResource>`), but it should not be the thing 52 `@<Resource>` references
resolve to.
...
### 4. `<Xhtml>` is referenced but never declared
...
### 5. (minor) `ABSTRACT` is never emitted
...
### 6. (cosmetic) a comment is glued to a closing brace
...
With 1–4 applied, the schema parses and validates. I'm not yet reporting
conformance numbers for the examples: with `<All>` corrected, most examples
report nonconformant, and I haven't finished attributing that between the
schema, the examples, and ShEx.js. (Minimal RDF-list matching against
`OneOrMore_*`-shaped constraints checks out, so it isn't that.) I'll follow up
