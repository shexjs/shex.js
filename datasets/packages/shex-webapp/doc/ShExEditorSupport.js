...
class EditorSupport {
    ...
    completionSets(language, cache) {
        ...
        const prefixes = Object.assign({}, inputData.meta && inputData.meta.prefixes, inputSchema.meta && inputSchema.meta.prefixes, cache.meta && cache.meta.prefixes, ...schemas.map(s => s._prefixes || {}));
    ...
    locateSchema() {
        ...
        if (!inputSchema.parsed || typeof inputSchema.parsed !== "object")
            return null;
        return ShExWebApp.EditorServices.locateInParsed(inputSchema.selection.val(), inputSchema.parsed, { base: inputSchema.meta && inputSchema.meta.base });
    }
    ...
    reportValidation(entries) {
        ...
        try {
            ...
            const dataDocuments = [{ at: documents.length ? showing : -1,
                    parsed: locate(inputData.selection.val()) }].concat(documents.map((d, at) => ({ at, parsed: at === showing ? null : locate(d.text) })))
                .filter(d => d.parsed);
            ...
            entries.forEach(entry => {
                ...
                const perDocument = dataDocuments.map(d => ({
                    ...
                    mapped: ShExWebApp.EditorServices.mapValidationErrors(entry.appinfo, located, d.parsed, { spelling: termSpelling() }),
                }));
                ...
                mapped.pairs.forEach((pair, i) => {
                    ...
                    if (pair.anchors && pair.anchors.object)
                        return;
                    const elsewhere = perDocument.slice(1).find(d => (d.mapped.pairs[i] || {}).anchors && d.mapped.pairs[i].anchors.object);
                    if (elsewhere) {
                ...
                if (!this.expectsNonconformant(entry)) {
                    ...
                    perDocument.forEach(d => {
                        ...
                        if (anchored)
                            ...
                            dataOf(d.at).push(Object.assign({ severity: "error", message }, (anchored.anchors.subjectParts || [anchored.anchors.subject])[0]));
                    });
    ...
    reaimAtShowingDocument() {
        ...
        if (this.panes.inputData)
            this.panes.inputData.setDiagnostics(byDoc ? (byDoc.get(showing) || [])
    ...
    allPairs() {
        return Object.keys(this.linkSets || {}).reduce((acc, id) => acc.concat(id === "validation"
            ? (this.linkSets[id] || [])
    ...
    setPairHovers(validationPairs) {
        ...
        const show = (group, hoveredSide, pinning) => {
            ...
            const dataRanges = group.flatMap(p => [].concat(anchorRanges(p, "object"), anchorRanges(p, "subject"), anchorRanges(p, "predicate")));
        ...
        if (dataPane) {
            ...
            const byTermRange = new Map();
            pairs.filter(shownHere).filter(p => !p.secondary).forEach(p => [].concat(anchorRanges(p, "object"), anchorRanges(p, "predicate")).forEach((r) => {
                const key = r.from + "-" + r.to;
                if (!byTermRange.has(key))
                    byTermRange.set(key, { range: r, group: [] });
                byTermRange.get(key).group.push(p);
            }));
            dataPane.setHoverRegions([...byTermRange.values()].map(({ range, group }) => ({
                from: range.from, to: range.to,
                enter: () => show(group, "data"),
                click: freeze(group, "data"),
                title: () => this.pairTitle(group, "data"),
            }))
                .concat(pairs.filter(shownHere).filter(p => p.secondary).flatMap(p => [].concat(anchorRanges(p, "object"), anchorRanges(p, "predicate"))
        ...
        linkedPanes.forEach(name => {
            ...
            pane.setHoverRegions([...byRange.values()].map(({ range, group }) => ({
                from: range.from, to: range.to,
        ...
        resultPanes.forEach(({ pane, ranges }) => {
            if (!pane.setHoverRegions)
                return;
            pane.setHoverRegions(ranges.reduce((acc, r) => {
                const pair = pairs.find(p => p.triple === r.target);
                return pair
                    ? acc.concat(termRanges(r).map(f => ({ from: f.from, to: f.to,
    ...
    resultTripleText(p) {
        ...
        try {
            ...
            const say = (term, aForType) => ShExWebApp.ShExTerm.shExJsTerm2Turtle(term, spell, aForType);
