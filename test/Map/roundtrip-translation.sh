...
SHEX_BIN_PATH=${CWD}/../../bin
EXT_BIN_PATH=${CWD}/../../extensions/shex-map/bin
...
while [[ $# -gt 0 ]]; do
  ...
  case "$opt" in
    ...
    "-j"|"--jsonvars"        ) JSON_VARS_FILE="$1"; 
                               ...
                               shift;;
    "-o"|"--output"          ) WORK_DIR="$1"; 
                               check_required_arg "${WORK_DIR}" "${opt} specified with no output directory!"
                               shift;;
    "-r"|"--root"            ) RDF_ROOT="$1"; 
...
if [ ! -f "${BACK_TARGET_TRANSLATION_FILE}" ]; then
  ...
  if [ "${SED_FILES}" = true ]; then 
    ...
    if [ "$(uname)" == "Darwin" ]; then
      ...
      echo -e "${YELLOW_FONT}WARNING: unable to update file ${BACK_TARGET_TRANSLATION_FILE} for back translation." >&2
      echo -e "${DEFAULT_COLOR}"
    fi
...
if [[ -z ${FRAME_FILE} ]]; then 
  ...
  RDF_TO_JSON="${SHEX_BIN_PATH}/rdf-to-json -r \"${BACK_TTL_FILE}\" > "${BACK_JSON_FILE}""
else 
  BACK_JSON_FILE="${WORK_DIR}/back-${BASE_SCHEMA_FNAME}.jsonld"
  RDF_TO_JSON="${SHEX_BIN_PATH}/rdf-to-json -r \"${BACK_TTL_FILE}\" -f \"${FRAME_FILE}\" > "${BACK_JSON_FILE}""
fi
