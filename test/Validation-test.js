           ...
           function (report) {                                             // test action
             var absoluteVal = valFile ? parseJSONFile(__dirname + "/" + valFile, function (k, obj) {
               ...
               if (["shape", "reference", "valueExprRef", "node", "subject", "predicate", "object"].indexOf(k) !== -1 &&
                   typeof obj[k] !== "object" &&
                   ShExCore.OldN3Util.isIRI(obj[k])) {
                 obj[k] = ShExCore.OldN3Util.resolveRelativeIRI(["shape", "reference", "valueExprRef"].indexOf(k) !== -1 ? schemaURL : dataURL, obj[k]);
               } else if (["values"].indexOf(k) !== -1) {
                 for (var i = 0; i < obj[k].length; ++i) {
                   if (typeof obj[k][i] !== "object" && ShExCore.OldN3Util.isIRI(obj[k][i])) {
                     obj[k][i] = ShExCore.OldN3Util.resolveRelativeIRI(dataURL, obj[k][i]);
                   }
                 };
               }
                  ...
                  if (error) {
                    ...
                    try {
                      function maybeGetTerm (base, s) {
                        ...
                        s.substr(0, 2) === "_:" ? s :
                          ShExCore.OldN3Util.resolveRelativeIRI(base, s);
                      }
