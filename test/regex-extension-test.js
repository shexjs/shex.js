...
describe('Regex extension', function() {
    ...
    describe('#lift', function() {
        ...
        it('should throw an error if there is no capture variable in the regular expression', function() {
            expect(regexExtension.lift.bind(this,
                                            "regex(/test/)",
                                            "A test string",
                                            {"test": "urn:local:test:"},
                                            "/test/")).to.throw(Error,
        ...
        it('should throw an error if there is no match in the regular expression', function() {
            expect(regexExtension.lift.bind(this,
                                            ...
                                            "A test string", 
                                            {test: 'urn:local:test:'},
                                            "/A (?<test:string>[0-9]+) string/")).to.throw(Error, 
                                                'regex(/A (?<test:string>[0-9]+) string/) found no match for input "A test string"!');
        });

        it('should execute a simple single variable regex function', function() {
            expect(regexExtension.lift("regex(/(?<dem:family>^[a-zA-Z]+)/)", 
                                       "Smith, Jane", 
                                       {"dem": "http://a.example/dem#"},
                                       "/(?<dem:family>^[a-zA-Z]+)/")).to.deep.equal(
                                       { 'http://a.example/dem#family': 'Smith'});
        });

        it('should execute a regex function with an embedded capture expression', function() {
            expect(regexExtension.lift("regex(/A (?<test:string>[a-zA-Z]+) string/)", 
                                       "A test string", 
                                       {test: 'urn:local:test:'},
                                       "/A (?<test:string>[a-zA-Z]+) string/")).to.deep.equal(
                                           { 'urn:local:test:string': 'test' });
        });

	it('should execute a regex function with a multiple capture variables', function() {
            expect(regexExtension.lift("regex(/(?<dem:family>[a-zA-Z]+), (?<dem:given>[a-zA-Z]+)/)",
                                       "Doe, John", 
                                       {"dem": "http://a.example/dem#"},
                                       "/(?<dem:family>[a-zA-Z]+), (?<dem:given>[a-zA-Z]+)/")).to.deep.equal(
    ...
    describe('#lower', function() {
        it('should throw an error if there is no capture variable in the regular expression', function() {
            expect(regexExtension.lower.bind(this, 
                                             "regex(/test/)",
                                             {"urn:local:test:string": "test"}, 
                                             {"test": "urn:local:test:"},
                                             "/test/")).to.throw(Error,
        ...
        it('should throw an error if there is no match in the regular expression', function() {
            expect(regexExtension.lower.bind(this,
                                             ...
                                             {}, 
                                             {"test": "urn:local:test:"},
                                             "/?<dem:family>^[a-zA-Z]+)/")).to.throw(Error, 
                                                  'Found no capture variable in regex(/A (?<test:string>[0-9]+) string/)!');
        });

        it('should execute a simple single variable regex function', function() {
            expect(regexExtension.lower("regex(/(?<dem:family>^[a-zA-Z]+)/)", 
                                        {"http://a.example/dem#family": "Smith"},
                                        {"dem": "http://a.example/dem#"},
                                        "/(?<dem:family>^[a-zA-Z]+)/")).to.equal("Smith");
        });

        it('should execute a regex function with an embedded capture expression', function() {
            expect(regexExtension.lower("regex(/A (?<test:string>[a-zA-Z]+) string/)", 
                                        {"urn:local:test:string": "test"}, 
                                        {"test": "urn:local:test:"},
                                        "/A (?<test:string>[a-zA-Z]+) string/")).to.equal("A test string");
        });

	it('should execute a regex function with a multiple capture variables', function() {
            expect(regexExtension.lower("regex(/(?<dem:family>[a-zA-Z]+), (?<dem:given>[a-zA-Z]+)/)",
                                        { 'http://a.example/dem#family': 'Doe', 'http://a.example/dem#given': 'John'},
                                        {"dem": "http://a.example/dem#"},
                                        "/(?<dem:family>[a-zA-Z]+), (?<dem:given>[a-zA-Z]+)/")).to.equal("Doe, John");
        });
    });
