...
var decimalLexicalTests = {
  ...
  fractiondigits: function (v, d) {
    ...
    return m && m[1].length <= d;
        ...
        function ldify (term) {if (!term) return "N/A"
          if (term[0] !== "\"")
            return term;
...
function ShExValidator_constructor(schema, options) {
  ...
  this.validate = function (db, point, label, tracker, seen, subGraph) {
    ...
    if (seen === undefined)
      seen = {};
    var seenKey = (typeof point === "string" ? point : JSON.stringify(point)) + "@" + (label === Start ? "_: -start-" : label);
    if (!subGraph) {
      if (seenKey in seen)
  ...
  this._validateShapeExpr = function (db, point, shapeExpr, shapeLabel, depth, tracker, seen, subgraph) {
    if (typeof point === "string" || !("type" in point)) {
      return this._validateShapeExpr1(db, point, shapeExpr, shapeLabel, depth, tracker, seen, subgraph);
    // if (typeof point === "string") { // ShapeRef !!!
    //   return this._validateShapeDecl(db, point, index.shapeExprs[point], point.reference, depth, tracker, seen, subgraph);
    } else if (point.type === "NodeConstraint") {
      return this._validateShapeExpr1(db, point, shapeExpr, shapeLabel, depth, tracker, seen, subgraph);
    } else if (point.type === "Shape") {
      return this._validateShapeExpr1(db, point, shapeExpr, shapeLabel, depth, tracker, seen, subgraph);
    } else if (point.type === "ShapeExternal") {
      return this._validateShapeExpr1(db, point, shapeExpr, shapeLabel, depth, tracker, seen, subgraph);
      return this.options.validateExtern(db, point, shapeLabel, tracker, seen);
    } else if (point.type === "ShapeOr") {
      var errors = [];
      for (var i = 0; i < point.shapeExprs.length; ++i) {
        var nested = point.shapeExprs[i];
        var sub = this._validateShapeExpr(db, nested, shapeExpr, shapeLabel, depth, tracker, seen, subgraph);
        if ("errors" in sub)
          errors.push(sub);
        else
  ...
  this._validateShapeExpr1 = function (db, point, shapeExpr, shapeLabel, depth, tracker, seen, subgraph) {
    if (point === "")
    ...
    if (typeof shapeExpr === "string") { // ShapeRef
      ...
      for (var i = 0; i < shapeExpr.shapeExprs.length; ++i) {
        ...
        var sub = this._validateShapeExpr1(db, point, nested, shapeLabel, depth, tracker, seen, subgraph);
        if ("errors" in sub)
          errors.push(sub);
        else
          return { type: "ShapeOrResults", solution: sub };
      ...
      var sub = this._validateShapeExpr1(db, point, shapeExpr.shapeExpr, shapeLabel, depth, tracker, seen, subgraph);
      if ("errors" in sub)
          return { type: "ShapeNotResults", solution: sub };
        else
          return { type: "ShapeNotFailure", errors: sub };
  ...
  this._validateShape = function (db, point, shape, shapeLabel, depth, tracker, seen, subgraph) {
    ...
    var outgoing = indexNeighborhood(fromDB.outgoing.sort(
      (l, r) => l.predicate.localeCompare(r.predicate) || sparqlOrder(l.object || l.valueExpr, r.object || r.valueExpr)
    ));
    ...
    while (misses.length === 0 && xp.next() && ret === null) {
      ...
      function passScoped (expr, extendsToTriples, valParms) {
        ...
        for (var eNo = 0; eNo < expr.extends.length; ++eNo) {
          ...
          var sub = _ShExValidator._errorsMatchingShapeExpr(
            false && "schema" in valParms.db && typeof point === "string" ? { type: "ShapeRef", reference: point } : point, // !!!
            extend, valParms, subgraph);
          if ("errors" in sub)
  ...
  this._triplesMatchingShapeExpr = function (triples, constraint, valParms) {
    ...
    triples.forEach(function (triple) {
      var value = triple.type === "TripleConstraint" ? triple.valueExpr : constraint.inverse ? triple.subject : triple.object;
      var sub;
  ...
  this._errorsMatchingShapeExpr = function (value, valueExpr, valParms, subgraph) {
    var _ShExValidator = this;
    if (typeof value === "string") { // ShapeRef
      if (false && index !== valParms.db.schema._index) {
        console.warn(index, " !== ", valParms.db.schema._index)
        throw new Error("better inspect just below here")
      }
      if ("schema" in valParms.db) {
      let expr = valParms.db.schema._index.shapeExprs[value]
      if (expr.type === "ShapeDecl")
        expr = expr.shapeExpr;
      return _ShExValidator._errorsMatchingShapeExpr(expr, valueExpr, valParms, subgraph);
      } else {
        return _ShExValidator._errorsMatchingShapeExpr1(value, valueExpr, valParms, subgraph);
      }
    } else if (value.type === "NodeConstraint") {
      return this._errorsMatchingShapeExpr1(value, valueExpr, valParms, subgraph);
    } else if (value.type === "Shape") {
      return this._errorsMatchingShapeExpr1(value, valueExpr, valParms, subgraph);
    } else if (value.type === "ShapeOr") {
      var errors = [];
      for (var i = 0; i < value.shapeExprs.length; ++i) {
        var nested = value.shapeExprs[i];
        var sub = _ShExValidator._errorsMatchingShapeExpr(nested, valueExpr, valParms, subgraph);
        if ("errors" in sub)
          errors.push(sub);
        else
          return { type: "ShapeOrResults", solution: sub };
      ...
      return { type: "ShapeAndResults", solutions: passes };
    } else if (value.type === "ShapeNot") {
      var sub = _ShExValidator._errorsMatchingShapeExpr(value.shapeExpr, valueExpr, valParms, subgraph);
      // return sub.errors && sub.errors.length ? {} : {
      ...
      throw Error("unknown value expression type '" + value.type + "'");
    }
  };

  this._errorsMatchingShapeExpr1 = function (value, valueExpr, valParms, subgraph) {
    var _ShExValidator = this;
    if (typeof valueExpr === "string") { // ShapeRef
      ...
      for (var i = 0; i < valueExpr.shapeExprs.length; ++i) {
        ...
        var sub = _ShExValidator._errorsMatchingShapeExpr1(value, nested, valParms, subgraph);
        if ("errors" in sub)
          errors.push(sub);
        else
          return { type: "ShapeOrResults", solution: sub };
      ...
      var sub = _ShExValidator._errorsMatchingShapeExpr1(value, valueExpr.shapeExpr, valParms, subgraph);
      // return sub.errors && sub.errors.length ? {} : {
      //   errors: ["Error validating " + value + " as " + JSON.stringify(valueExpr) + ": expected NOT to pass"] };
      var ret = Object.assign({
        type: null,
  ...
  this._errorsMatchingNodeConstraint = function (value, valueExpr, recurse) {
    ...
    if (false) {
      ...
      if (valueExpr.datatype) {
        if (value.type === "NodeConstraint") {
          if ("datatype" in value && value.datatype !== valueExpr.datatype) {
            validationError("mismatched datatype expressions: " + value.datatype + " !== " + valueExpr.datatype);
          }
          if ("nodeKind" in value && value.nodeKind !== "literal") {
            validationError("mismatched datatype expressions: " + value.nodeKind + " can never have datatype " + value.datatype);
          }
        }
        else if (!RdfTerm.isLiteral(value)) {
      ...
      if (valueExpr.values) {
        if (typeof value === "object" && value.type === "NodeConstraint") {
          if (valueExpr.values.filter(
            l => "values" in value
              ? value.values.filter(
                r => l.value === r.value
              ).length
              : "nodeKind" in value
              ? value.nodeKind === "iri" && RdfTerm.isIRI(l)
              : true
          ).length === 0)
            validationError("NC values " + JSON.stringify(value.values) + " not found in set " + JSON.stringify(valueExpr.values));
        } else if (RdfTerm.isLiteral(value) && valueExpr.values.reduce((ret, v) => {
          if (ret) return true;
          var ld = ldify(value);
          if (v.type === "Language") {
...
function sparqlOrder (l, r) {
  if (l.type === "NodeConstraint" || r.type === "NodeConstraint") return 0;
