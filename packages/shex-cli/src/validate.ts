...
import ShExUtil = require("@shexjs/util");
 ...
 * options: one that declares them (dbParams) and builds from them (fromParams) */
type QueryDbModule = NeighborhoodModule & Required<Pick<NeighborhoodModule, "dbParams" | "fromParams">>;
...
const QueryDbModules: QueryDbModule[] = [
...
/** the exit statuses in lib/ExitCode.js */
interface ExitCodes {
  help: number;
  dry_run: number;
  shape_test_pass: number;
  shape_test_fail: number;
  bad_argument: number;
  graph_match_pass: number;
  graph_match_fail: number;
  val_match_pass: number;
  val_match_fail: number;
  json_pass: number;
  shexc_pass: number;
  term_not_found: number;
  resource_not_found: number;
  file_not_found: number;
  manifest_error: number;
  manifest_not_found: number;
  unspecified_error: number;
}

/** what command-line-args parses out of CommandLineOptions.  The neighborhood
 * modules' options (--endpoint, --wikibase, ...) arrive under whatever names
 * their dbParams declare, hence the index signature. */
interface Cmds {
  verbose?: boolean;
  quiet?: boolean;
  terse?: boolean;
  ...
  list?: boolean;
  track?: boolean;
  /** --slurp-all sets this to the string "true" (loadCommandLine) */
  ...
  node?: string;
  "node-type"?: string;
  /** -m as typed; loadSchemaAndData replaces it with the parsed shape map */
  queryMap?: string | { node: any, shape: any }[];
  mapURL?: string;
  shape?: string;
  schemaURL: string[];
  json: string[];
  ...
  jsonld: string[];
  extension: string[];
  /** a file|URL, or "true"/"false" which loadCommandLine turns into booleans */
  ...
  "turtle-manifest": string[];
  "jsonld-manifest": string[];
  "test-name": string[];
  invocation?: boolean;
  "dry-run"?: boolean;
  "regex-module"?: string;
...
/** what command-line-args wants: a defaultOption (the bare arguments) on top
 * of what neighborhood-api's translation covers */
interface CliOption extends CliOptionDefinition {
  defaultOption?: boolean;
}

/** options for the ShExValidator constructor; a manifest's traits add keys
 * by name (TraitToOption), hence the index signature */
...
/** one --provenance data file: its lezer-turtle provenance index and diagnostics */
...
/** what a validation request to the server accumulates (mergeParm), keyed
 * by request parameter name */
interface ServerParms {
  schema: any;
  shape?: string;
  schemaMeta: any[];
  data: any;
...
const TraitToOption: { [trait: string]: string } = { eachOf: "or", oneOf: "or", someOf: "or", exhaustive: "coverage", firstError: "coverage" };

const CommandLineOptions: CliOption[] = [
...
for (const mod of QueryDbModules)
  for (const opt of paramsToCommandLineArgs(mod.dbParams || []))
...
function queryDbParams (mod: NeighborhoodModule, cmds: Cmds): { [name: string]: any } | null {
  const specs = mod.dbParams || [];
  const cliName = (s: DbParamSpec) => s.cli && s.cli.option || s.name;
...
function selectedQueryDbModule (cmds: Cmds): QueryDbModule | null {
  return QueryDbModules.find(mod => queryDbParams(mod, cmds) !== null) || null;
}

async function makeQueryDb (cmds: Cmds, queryTracker: DbQueryTracker | undefined): Promise<NeighborhoodDb | null> {
  ...
  for (const spec of (mod.dbParams || []).filter(
    s => s.pane && params[s.name] !== undefined && s.schema.items?.format === "uri"))
    params[spec.name] = await Promise.all(
      ([] as string[]).concat(params[spec.name]).map(ref => ShExNode.GET(ref).then(body => body.text)));
...
function abort (msg: string, exitCode: number): never {
 ...
 */
async function findNodesAndValidate (loaded: any, parms: any, options: any, schemaStart: any, cmds: Cmds) {
  let shapeIndex = loaded.schema._shapes || ShExIndexVisitor.index(loaded.schema)
  function knownShape (label: string) {
    return label in shapeIndex.shapeExprs;
  }
  function unknownShape (label: string): never {
  ...
  function resolveShapeMapExtension (language: string, lexical: string) {
    const mod = selectedQueryDbModule(parms) || RdfJsModule;
    const resolver = queryMapResolverFor(mod, language);
    if (!resolver)
      throw new TermError(`the QueryMap extension ${extensionName(language)} is not supported ` +
                          `by the neighborhood ${moduleId(mod)}`);
    // this walk is synchronous, so a resolver had better answer outright
    return (resolver.resolve(lexical, loaded.data) as RdfJsTerm[]).map(ShExTerm.rdfJsTerm2Ld);
  }

  function _makeShapeMap (nodeP: any, shapeP: any, nodeTypeP: any, schemaStart: any) {
    ...
    if (nodes[0] === undefined || (shape === undefined && schemaStart === undefined)) {
      ...
      if (nodes[0] === undefined) {
        const subjectNodes = loaded.data.size < 50 ? Object.keys(loaded.data.getQuads(null, null, null).reduce(function (r: any, t: any) { r[t.subject] = t.subject; return r; }, {})) : [];
    ...
    return nodes.map((node: any) => ({ node: node, shape: shape }));
  }

  const ret: { shape?: any, results?: any } = {};

  const shapeMap = "queryMap" in parms
      ? parms.queryMap.reduce(
        (acc: any, pair: any) => acc.concat(_makeShapeMap(pair.node, pair.shape, null, schemaStart)), []
      ).reduce(
        (acc: any, pair: any) => acc.seen.indexOf(pair.node) === -1
  ...
  if (typeof ret.results === 'object' && !("errors" in ret.results)) {
    ...
    if (cmds.remainder) {
      ...
      ret.results.graph.getQuads().forEach((q: any) => remainder.removeQuad(q));
      ret.results.graph = remainder;
    }
  }
  return ret;

  function optsStr (key: string, dict: any) {
    const r = RegExp(key.substr(key.lastIndexOf("/")).toLowerCase(), "i");
    const opts = Object.keys(dict).reduce(function (ret: string[], k) {
      if (k.match(r))
        ret.push(k);
      return ret;
    }, []);
...
async function loadSchemaAndData (valParms: any, validatorOptions: any, schemaOptions: any, cmds: Cmds) {
  ...
  const errors: { error: any, yylloc: any }[] = [];
  ...
  try {
    ...
    const queryTracker = valParms.track || valParms.slurp
          ...
          : undefined;
    let provSources: ProvSource[] | null = null;
    if (valParms.provenance) {
      ...
      const {parseTurtle} = await import("lezer-turtle/emit");
      const store = new N3.Store();
      provSources = await Promise.all(valParms.dataURL.map(async (u: string) => {
        const body = await ShExNode.GET(u);
        const parsed = parseTurtle(body.text, {factory: N3.DataFactory, baseIRI: body.url});
        parsed.quads.forEach((q: any) => store.addQuad(q));
        return {source: body.url, provenance: parsed.provenance, diagnostics: parsed.diagnostics};
      }));
      schemaAndData.data = store;
    }
    schemaAndData.data = (await makeQueryDb(valParms, queryTracker))
    ...
    if (valParms.diagnose) {
      ...
      if (redefined.length > 0) {
        ...
        redefined.forEach(id => {
          ...
          console.log('  %s: %d defintions:%s', ShExTerm.shExJsTerm2Turtle(id), llocs.length, llocs.map((yylloc: any) => "\n    " + yyllocToString(yylloc)).join(""));
        });
      }
      if (errors.length > 0)
        ...
        if (res.graph) {
          ...
          console.log(
            cmds["human"]
              ...
              ? (res.solutions || [res]).map((s: any) => `${s.node}@${s.shape}`).join("\n")
  ...
  function makeQueryTracker (schemaMeta: any, dataMeta: any): DbQueryTracker {
    if (valParms.slurp) {
  ...
  function metaToResolver (meta: any) {
    return meta
 ...
 */
function attachProvenance (result: any, provSources: ProvSource[]) {
  walk(result);
  function walk (node: any) {
    ...
    if (node.type === "TestedTriple" && "subject" in node) {
      const quad = N3.DataFactory.quad(ShExTerm.ld2RdfJsTerm(node.subject),
                                       ShExTerm.ld2RdfJsTerm(node.predicate),
                                       ShExTerm.ld2RdfJsTerm(node.object));
      const utterances = provSources.flatMap(
        ({source, provenance}) => provenance.get(quad).map(
          (utt: any) => ({source,
...
function rdflib_termToLex (node: any, resolver: any) {
...
function getInvocation (parms: any) {
  function shortenFile (u: string) {
...
*/
async function queueTests (tests: any[], cmds: Cmds) {
  ...
  return results.reduce(function (worst: number, result: number) {
    return worst < result ? result : worst;
  }, 0);
}

async function runValidator (db: any, shapeMap: any, schema: any, options: any, cmds: Cmds) {
  // prepare validator
...
async function loadRDFmanifest (cmds: Cmds) {
  ...
  try {
    ...
    const testNodes = cmds["test-name"].length ?
        cmds["test-name"].reduce((ret: any[], testName) => {
          ...
          loaded.data.getQuads(null, mf+"name", null).filter(function (triple: any) {
            if (triple.object.value.match(pattern))
              ret.push(triple.subject);
          });
          return ret;
        }, []) :
        loaded.data.getQuads(null, mf+"action", null).map(function (t: any) {
          return t.subject;
        });
    const tests = testNodes.map(function (subject: any) {
      ...
      return [["schema", "schemaURL", true ],
              ...
              ["data"  , "dataURL"  , true ],
              ["focus" , "node"     , false],
              ["map"   , "mapURL"   , false]].reduce(function (ret: any, elt: any) {
...
function injectCertainCommandLineArgs (run: any, cmds: Cmds) {
...
function loadJSONmanifest (jsonManifests: string[], parser: (text: string) => any, cmds: Cmds) {
  return jsonManifests.map(async function (jm) {
    let [curAct, errorCode]: [string, number] = ["load", ExitCode.manifest_not_found];
    try {
...
function loadCommandLine (cmds: Cmds) {
...
class FileCache {
  seen: { [path: string]: Promise<string> };
...
function runServer (serverLoaded: any, serverParms: any, serverOptions: any, cmds: Cmds) {
  const app       = new (require("koa"))();
  const {koaBody} = require("koa-body");
  const log: typeof console.log = serverParms.quiet ? () => undefined : console.log;
  ...
  async function koaHandler (ctx: any, next: () => Promise<void>) {
  ...
  async function serveKoaValidationRequest (ctx: any) {
      const parms: ServerParms = {
      ...
      const unlinkMe = await Promise.all(Object.keys(ctx.request.files || []).reduce((acc: any[], key) => {
      ...
      try {
        ...
        let result: any = await findNodesAndValidate(parms, parms, options, parms.schema.start, cmds);
...
function populateTemplate (templateStr: string, vars: { [name: string]: any }) {
  return Object.keys(vars).reduce((r, p) => {
    return r.replace("["+p+"]", vars[p]);
  }, templateStr)
}

async function mergeParm (parms: ServerParms, parm: string, text: string, url: URL, _type: any) {
  switch (parm) {
  case "schema":
    let part: any = null;
    const prefixes = {};
  ...
  case "data":
    const tpz = await new Promise((resolve, reject) => {
      ...
      parser.parse(text, function (error: any, quad: any, prefixes: any) {
        if (error) {
...
function makeError (msg: string, status: number) {
  const ret: Error & { status?: number } = Error(msg);
  ret.status = status;
  return ret;
}

function getRegexModule (module: any) {
  if (module) {
    try {
...
function extend (base: any, ...args: any[]) {
  if (!base) base = {};
  for (let i = 0, l = args.length, arg: any; i < l && (arg = args[i] || {}); i++)
    for (let name in arg)
      base[name] = arg[name];
  return base;
}
