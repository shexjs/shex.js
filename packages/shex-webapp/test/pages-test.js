  ...
  it("should have no rule for an id no page has", () => {
    ...
    for (const {file} of pages)
      for (const m of read(file).matchAll(/\bid="([^"]+)"/g))
        ids.add(m[1]);
    // ids the apps create as they go count as had
    for (const js of ["../doc/ShExBaseApp.js", "../../extension-map/doc/ShExMapBaseApp.js"])
      // including the ones written into markup the app builds as strings
      for (const m of read(js).matchAll(/\bid: ?"([^"]+)"|attr\("id", ?"([^"]+)"|id=\\?"([^"\\]+)/g))
        ids.add(m[1] || m[2] || m[3]);
