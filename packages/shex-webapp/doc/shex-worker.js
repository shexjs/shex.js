  ...
  const ret = {
    ...
    set: async function (text, base) {
      ...
      if (base !== DefaultBase) {
        ...
        // Note that App.Caches.manifest.set takes a 3rd parm.
      }
    },
...
function makeSchemaCache (selection) {
  ...
  ret.getItems = async function () {
    ...
    const rest = "shapes" in obj ? obj.shapes.map(se => App.Caches.inputSchema.meta.termToLex(se.id)) : [];
...
function makeTurtleCache (selection) {
  ...
  ret.getItems = async function () {
    ...
    return data.getQuads().map(t => {
      return App.Caches.inputData.meta.termToLex(t.subject); // !!check
    });
  };
  return ret;
}
    ...
    const demos = textOrObj.reduce((acc, elt) => {
      if ("action" in elt) { // TODO: move to ShExUtil
        ...
        const queryMap = "map" in action ?
            null :
              ldToTurtle(action.focus, App.Caches.inputData.meta.termToLex)
              + "@"
              + ("shape" in action ? App.Caches.inputSchema.meta.termToLex(action.shape, false) : START_SHAPE_LABEL);
...
`)()
      ...
      App.Caches.extension.url = url; // @@ cheesy hack that only works to remember one extension URL
      results.append($("<div/>").append(
