...
class EditorSupport {
    ...
    completionSets(language, cache) {
        ...
        const prefixes = Object.assign({}, inputData.meta && inputData.meta.prefixes, inputSchema.meta && inputSchema.meta.prefixes, cache.meta && cache.meta.prefixes);
    ...
    reportValidation(entries) {
        ...
        if (!this.panes.inputSchema || !inputSchema.parsed)
            return;
        try {
            const located = ShExWebApp.EditorServices.locateInParsed(inputSchema.selection.val(), inputSchema.parsed);
