 ...
 * ShExBaseApp.js.
 *
 * This is doc/ShExBaseApp-layout.js's source (tsconfig.app.json compiles src/app/ into
...
mixin(ShExBaseApp, {
    ...
    paneTabset(pane, screen, columns) {
        ...
        set.children("ul").first().append($("<li/>").append($("<a/>", { href: "#" + panel }).text(pane.label || pane.name)));
        return set;
    },
    ...
    setEditors() {
        ...
        if (want && !this.editorSupport) {
            ...
            // cache.
            Object.defineProperty(this.Caches, "editorSupport", { value: this.editorSupport, enumerable: false, configurable: true });
            this.addEditorPanes();
    ...
    async toggleControls(evt) {
        ...
        if (revealing)
            $(document).on("click.shexjsControls", (e) => {
    ...
    customizeInterface() {
        if ($("#interface").val() === "minimal") {
            ...
            $("#menuForm").css("position", "absolute").css("left", $("#inputSchema .status").get(0).getBoundingClientRect().width -
    ...
    async prepareDragAndDrop() {
        this.QueryParams.filter(q => {
            ...
            desc.location.
                ...
                if (prefTypes.find(l => {
                    if (l.type.indexOf("/") === -1) {
                        ...
                        if (xfer.getData(l.type)) {
                            ...
                            async function inject(targets, url, data, mediaType) {
                                const target = targets.length === 1 ? targets[0].target :
                    ...
                    this.resultsWidget.append($("<pre/>").text("drag and drop not recognized:\n" +
                        JSON.stringify({
        ...
        const readfiles = /*async*/ (files, targets) => {
