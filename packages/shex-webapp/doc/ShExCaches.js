...
class SchemaCache extends InterfaceCache {
    ...
    async parse(text, base) {
        const parseShExR = () => {
            const graphParser = new ShExWebApp.Validator(this.shexcParser.parseString(ShExWebApp.Util.ShExRSchema, {}, base), // !! do something useful with the meta parm (prefixes and base)
        ...
        const schema = isJSON ? ShExWebApp.Util.ShExJtoAS(JSON.parse(text)) :
...
class ManifestCache extends InterfaceCache {
    ...
    constructor(selection, caches, resultsWidget) {
        ...
        this.onPickInputs = null; // the app's, to end a debug session standing over inputs about to change
        this.caches = caches;
    ...
    async set(textOrObj, url, source) {
        ...
        const demos = textOrObj.reduce((acc, elt) => {
            if ("action" in elt) { // TODO: move to ShExUtil
                ...
                }, (queryMap ? { queryMap: queryMap } : { queryMapURL: queryMapURL }), { status: elt["@type"] === "sht:ValidationFailure" ? "nonconformant" : "conformant" });
...
class PluginCache extends InterfaceCache {
    ...
    static trust(origin) {
        try {
            window.sessionStorage.setItem(PluginCache.TRUSTED_ORIGINS_KEY, JSON.stringify(PluginCache.trustedOrigins().concat(origin)));
        }
        catch (e) {
...
`)();
            ...
            painted.forEach(d => { if (!d.baseUrl)
                d.baseUrl = url; });
            ...
            if (!handles) {
                this.resultsWidget.append($("<div/>").append($("<span/>").text(`plugin ${painted.map(d => d.label || d.id).join(", ")} loaded from <${url}>`)));
                return;
            }
            ...
            if (old.length) {
                this.resultsWidget.append($("<div/>").append($("<span/>").text(`removing old ${old.attr('data-name')} plugin`)));
                old.parent().remove();
            }
            // Create a new li.
            const elt = $("<li/>", { class: "menuItem", title: loaded.description }).append($("<input/>", {
                type: "checkbox",
                checked: "checked",
                class: "pluginControl",
                id: id,
                "data-name": name,
                "data-url": loaded.url
            }), $("<label/>", { for: id }).append($("<a/>", { href: loaded.url, text: name })));
            elt.insertBefore("#load-plugin-button");
            $("#" + id).data("code", loaded);
            this.resultsWidget.append($("<div/>").append($("<span/>").text(`plugin ${name} loaded from <${url}>`)));
        }
        catch (e) {
    ...
    */
    async grepHtmlIndexForPackage(code, url, source) {
        ...
        if (impls.length !== 1) {
            this.resultsWidget.append($("<div/>").append($("<span/>").text("unparsable extension index at " + url)).addClass("error"));
            return;
        }
        const tr = $(impls).find(`tr td[resource="${ShExJsUrl}"]`).parent();
        if (tr.length !== 1) {
            this.resultsWidget.append($("<div/>").append($("<span/>").text("no entry for shexjs in index HTML at " + url)).addClass("error"));
            return;
        }
        const href = tr.find('[property="shex:package"]').attr('href');
        if (!href) {
            this.resultsWidget.append($("<div/>").append($("<span/>").text("no package for shexjs in index HTML at " + url)).addClass("error"));
            return;
        }
        const refd = await fetch(href);
        if (!refd.ok) {
            this.resultsWidget.append($("<div/>").append($("<span/>").text(`error fetching implementation: ${refd.status} (${refd.statusText}) for URL <${href}>`)).addClass("error"));
        }
        else {
