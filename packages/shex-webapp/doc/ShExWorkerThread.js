        ...
        switch (msg.data.request) {
            case "create":
                ...
                const inputData = msg.data.neighborhood
                    ? asyncFace(neighborhoodModule(msg.data.neighborhood), msg.data.params, msg.data.slurp ? queryTracker() : null)
                ...
                validator = new ShExWebApp.Validator(msg.data.schema, inputData, createOpts);
            ...
            case "debugValidate": {
                ...
                const dbgDb = ShExWebApp.RdfJsDb(makeStaticDB((msg.data.data || []).map((t) => WorkerMarshalling.jsonTripleToRdfjsTriple(t, N3js.DataFactory))));
                ...
                const gate = new ShExWebApp.WorkerGate(msg.data.sab, (m) => self.postMessage({ response: "paused", event: m.event }), ShExWebApp.schemaTripleConstraints(msg.data.schema));
                ...
                const dbgMap = (msg.data.queryMap || []).map((m) => ShExWebApp.ShExTerm.isStart(m.shape) ? Object.assign({}, m, { shape: ShExWebApp.Validator.Start }) : m);
                try {
