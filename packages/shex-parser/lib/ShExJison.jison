...
tripleConstraint:
	...
	if ($3 !== EmptyShape && false) {
	  ...
	  $3 = t; // ShapeRef
        ...
        $$ = extend({ type: "TripleConstraint" }, $1 ? $1 : {}, { predicate: $2 }, ($3 === EmptyShape ? {} : { valueExpr: $3 }), $4, $6); // t: 1dot // t: 1inversedot, 1negatedinversedot
        if ($5.length)
...
accessor:
    ...
    | IT_LANGTAG '(' '$' tripleExprLabel ')'	-> { type: "LangtagAccessor", name: $4 }
    | IT_DATATYPE '(' '$' tripleExprLabel ')'	-> { type: "DatatypeAccessor", name: $4 }
    ;
