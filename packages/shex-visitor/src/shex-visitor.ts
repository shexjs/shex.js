function ShExVisitor() {
  function isTerm(t) {
    return (
      typeof t !== "object" ||
      ("value" in t &&
        Object.keys(t).reduce((r, k) => {
          return r ? r : ["value", "type", "language"].indexOf(k) !== -1;
        }, true))
    );
  }
  ...
  const r: any = {
    ...
    visitShapes: function (shapes) {
      ...
      return shapes.map((shapeExpr) => _Visitor.visitShapeExpr(shapeExpr));
    },

    visitProductions999: function (productions) {
      // !! DELETE
      const _Visitor = this;
    ...
    visitShapeExternal: function (expr) {
      ...
      return Object.assign("id" in expr ? { id: expr.id } : {}, {
        type: "ShapeExternal",
      });
    },
    ...
    visitExpression: function (expr) {
      ...
      const r =
        expr.type === "TripleConstraint"
          ...
          : null;
      if (r === null) throw Error("unexpected expression type: " + expr.type);
      else return r;
    },
    ...
    visitStemRange: function (t) {
      ...
      if (!("type" in t))
        _Visitor.runtimeError(
          Error(
            "expected " + JSON.stringify(t) + " to have a 'type' attribute."
          )
        );
    ...
    _maybeSet: function (obj, ret, context, members, ignore) {
      ...
      this._testUnknownAttributes(
        obj,
        ignore ? members.concat(ignore) : members,
        context,
        this._maybeSet
      );
  ...
  r.visitBase = r.visitStart = r.visitClosed = r["visit@context"] =
    r._visitValue;
  r.visitExtra = r.visitAnnotations = r._visitList;
  r.visitInverse = r.visitPredicate = r._visitValue;
  r.visitName = r.visitId = r.visitCode = r.visitMin = r.visitMax =
    r._visitValue;

  r.visitType = r.visitNodeKind = r.visitDatatype = r.visitPattern = r.visitFlags = r.visitLength = r.visitMinlength = r.visitMaxlength = r.visitMininclusive = r.visitMinexclusive = r.visitMaxinclusive = r.visitMaxexclusive = r.visitTotaldigits = r.visitFractiondigits =
    r._visitValue;
