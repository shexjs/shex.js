...
function sparqlDB(endpoint, queryTracker, options = {}) {
    ...
    function getNeighborhood(point, shapeLabel, shape) {
        ...
        const outgoing = (tcs.out.length > 0 || shape.closed)
            ? mapQueryToTriples(shape.closed || options.allOutgoing
                ? `SELECT ?p ?o { ${pointStr} ?s ?p ?o }`
                : `SELECT ?p ?o { # ${point}\n` + pointStr +
                    pz.map(p => `  {?s <${p}> ?o BIND(<${p}> AS ?p)}`).join(" UNION\n") +
                    "\n}", point, null)
            : [];
        if (queryTracker) {
            const time = new Date();
...
${find(bnodesByPredicate[p][0])}  ?s ?p ?o
    ...
    function find(point, depth = 0, recursed = false) {
        ...
        const uniqueStr = unique
            ? ".\n" + Object.keys(unique).map(p => `  ${s} <${p}> ${unique[p].map((o) => `<${o}>`).join(', ')} .
MINUS {
  ${s} <${p}> ${s}_ne .
        ...
        return [...index.sz].reduce((acc, s) => {
            if (s in duplicates) {
                ...
                for (const pz of pzIterator) {
                    ...
                    const conflicts = others.filter((other) => hashOfArraysEqual(testUnique, vals(index.pso, other, pz)));
