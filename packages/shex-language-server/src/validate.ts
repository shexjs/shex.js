...
export function validate (schemaText: string, dataText: string, shapeMapText?: string, base: string = BASE): ValidateResult {
  ...
  const pairs = parseFixedShapeMap(shapeMapText, base);
  if (pairs.length === 0)
 ...
 * (or relative `<rel>`, resolved against `base`) node, and a shape that is the
 * `START` keyword or an `<iri>`. */
export function toPair (nodeText: string, shapeText: string, base: string = BASE): { node: string; shape: unknown } {
  return { node: resolve(unwrap(nodeText), base), shape: /^start$/i.test(shapeText) ? Start : unwrap(shapeText) };
}
...
function parseFixedShapeMap (text: string | undefined, base: string): { node: string; shape: unknown }[] {
  ...
  for (const line of (text || "").split(/\n+/)) {
    ...
    if (at < 0) continue;
    pairs.push(toPair(t.slice(0, at).trim(), t.slice(at + 1).trim(), base));
  }
  return pairs;
}
