...
/** What a mouseover on a ShExC triple constraint should do: "highlight" (the
 * default) lets the client light the matching data triples -- so the server
 * gives no hover there, since the ShExJ tooltip would obscure the highlight;
 * "shexj" shows the constraint's ShExJ instead.  Set via the client's
 * `initializationOptions.schemaMouseover` and `shex.schemaMouseover` setting. */
let schemaMouseover: "highlight" | "shexj" = "highlight";
function readSchemaMouseover (v: unknown): void {
  if (v === "highlight" || v === "shexj") schemaMouseover = v;
}

// --- lifecycle --------------------------------------------------------------

connection.onInitialize((params: InitializeParams): InitializeResult => {
  readSchemaMouseover((params.initializationOptions as { schemaMouseover?: unknown } | undefined)?.schemaMouseover);
  return {
    capabilities: {
...
connection.onDidChangeConfiguration(change => {
  readSchemaMouseover((change.settings as { shex?: { schemaMouseover?: unknown } } | undefined)?.shex?.schemaMouseover);
});
...
connection.onHover(({ textDocument, position }): Hover | null => {
  ...
  if (at) {
    // In "highlight" mode the client lights the matching data triples on
    // mouseover; a ShExJ tooltip here would just cover them, so give none.
    if (schemaMouseover !== "shexj") return null;
    const e: any = at.expr;
