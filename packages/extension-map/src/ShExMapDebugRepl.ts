...
class ShExMapDebugRepl {
  ...
  showEvent (event: any, sourceOnly = false) {
    ...
    case "done": {
      ...
      (report.unboundVariables || []).forEach((f: any) =>
        this.write("warning: " + this.lex(f.variable) + " is bound nowhere\n"));
      (report.unusedStatics || []).forEach((k: any) =>
  ...
  showThreads (arg: any) {
    ...
    if (arg) {
      ...
      return this.write(all[n - 1].quads.map((q: any) => "  " + this.termStr(q.subject) + " " +
