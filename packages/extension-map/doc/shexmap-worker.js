...
function prepareValidator(loaded, schemaURL, inputData) {
  return new RemoteValidator(loaded, schemaURL, inputData);
}
  ...
  try {
    ...
    if (hasFocusNode()) {
      ...
      try {
        ...
        let time;
        const validator = await prepareValidator(loaded, alreadLoaded.url, inputData);
