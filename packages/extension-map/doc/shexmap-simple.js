...
// let ShExRSchema; // defined calling page
...
function callValidator (done) {
  ...
  try {
    ...
    if (hasFocusNode()) {
      ...
      const inputData = Caches.inputData.refresh(); // need prefixes for ShapeMap
...
function materialize () {
  ...
  try {
    ...
    const outputSchema = Caches.outputSchema.refresh();
    ...
    const resultBindings = _dup(Caches.bindings.refresh());
    if (Caches.statics.get().trim().length === 0)
      Caches.statics.set("{  }");
    const _t = Caches.statics.refresh();
      ...
      build: function (elt, evt) {
        if (elt.hasClass("data")) {
          ...
          if (m) {
            ...
            const store = Caches.inputData.refresh();
            const items = [];
            if (terms.match === null)
