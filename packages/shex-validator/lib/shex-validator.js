...
class ShExValidator {
     ...
     */
    repairedBy(feasibility, missing, tc, hi) {
        const granted = new Map(hi);
        missing.forEach(mtc => granted.set(mtc, Math.max(granted.get(mtc) || 0, mtc.min === undefined ? 1 : mtc.min || 1)));
