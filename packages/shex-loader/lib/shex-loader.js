...
function ShExLoaderCjsModule(config = {}) {
    const loader = {
        load: load,
    ...
    function mergeSchema(obj, mediaType, resourceLoadControler, options, importers) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!("schema" in obj))
    ...
    function mergeGraph(obj, mediaType, _resourceLoadControler, _options, importers) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
    ...
    function loadSchemaImports(schema, importers, resourceLoadControler, schemaOptions) {
        ...
        schema.imports.map((i) => "iriTransform" in schemaOptions ? schemaOptions.iriTransform(i) : i).filter((i) => resourceLoadControler.loadNovelUrl(i)).map((i) => {
            resourceLoadControler.add(loader.GET(i).then(loaded => {
                ...
                // metaList.push(meta)
                return parseShExC(loaded.text, "text/shex", loaded.url, meta, schemaOptions, resourceLoadControler, importers)
    ...
    function loadList(src, metaList, mediaType, parserWrapper, merger, options, resourceLoadControler, importers) {
        return __awaiter(this, void 0, void 0, function* () {
            return src.map(p => {
                ...
                if (typeof p === "string") {
                    ret = loader.GET(p, mediaType).then(loaded => {
                        ...
                        return parserWrapper(loaded.text, mediaType, loaded.url, meta, options, resourceLoadControler, importers);
                    });
                }
                else {
                    if ("text" in p) {
     ...
     * @constructor
     */
    function load(schema_1, data_1) {
        return __awaiter(this, arguments, void 0, function* (schema, data, schemaOptions = {}, dataOptions = {}) {
            const returns = {
            ...
            // gather all the potentially remote inputs
            {
                ...
                loadList(shexc, returns.schemaMeta, "text/shex", parseShExC, mergeSchema, schemaOptions, allSchemas, []);
                loadList(json, returns.schemaMeta, "application/json", parseShExJ, mergeSchema, schemaOptions, allSchemas, []);
                loadList(turtle || [], returns.schemaMeta, "text/turtle", parseShExR, mergeSchema, schemaOptions, allSchemas, []);
            }
            {
                const { turtle = [], jsonld = [] } = data || {};
                allGraphs = dataOptions.loadController || new ResourceLoadControler(turtle.concat(jsonld));
                loadList(turtle, returns.dataMeta, "text/turtle", parseTurtle, mergeGraph, dataOptions, allGraphs, []);
                loadList(jsonld, returns.dataMeta, "application/ld+json", parseJSONLD, mergeGraph, dataOptions, allGraphs, []);
            }
            const [schemaSrcs, dataSrcs] = yield Promise.all([allSchemas.allLoaded(),
                allGraphs.allLoaded()]);
            ...
            schemaSrcs.forEach((sSrc) => {
                const { schema } = sSrc, schemaMeta = __rest(sSrc, ["schema"]);
    ...
    function parseShExC(text, mediaType, url, meta, schemaOptions, resourceLoadControler, importers) {
        ...
        try {
            meta.prefixes = {};
            meta.importers = importers;
            const schema = parser.parse(text, url, { meta });
    ...
    function parseShExR(text, mediaType, url, meta, schemaOptions, resourceLoadControler, importers) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const x = yield parseTurtle(text, mediaType, url, meta, schemaOptions, resourceLoadControler, importers);
                ...
                yield loadSchemaImports(schema, importers.concat([url]), resourceLoadControler, schemaOptions || {}); // shouldn't be any
                return Promise.resolve({ mediaType, url, importers, schema });
            }
            catch (e) {
    ...
    function parseTurtle(text, mediaType, url, meta, _dataOptions, _resourceLoadControler, importers) {
        return new Promise(function (resolve, reject) {
            const graph = [];
            new config.rdfjs.Parser({ baseIRI: url, blankNodePrefix: "", format: "text/turtle" }).
                parse(text, function (error, quad, prefixes) {
                if (prefixes) {
    ...
    function parseJSONLD(text, mediaType, url, meta, dataOptions, resourceLoadControler, importers) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    function LoadNoExtensions(_globs) { return []; }
}
module.exports = ShExLoaderCjsModule;
//# sourceMappingURL=shex-loader.js.map