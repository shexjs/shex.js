import {rdfJsTerm2Ld} from "@shexjs/term";
 ...
 */
function compileNFA (schema: any, shape: any) {
    ...
    function NFA () {
      ...
      function walkExpr (expr: any, stack: any[]): { start: number, tail: number[] } {
        let s: number = 0, starts: number[];
        let lastTail: number[] = [];
        function maybeAddRept (start: number, tail: number[]) {
        ...
        if (expr.type === "TripleConstraint") {
          ...
          lastTail = [];
          starts = [];
          expr.expressions.forEach(function (nested: any, ord: number) {
      ...
      function State_make (c: any, outs: number[], negated?: boolean) {
  ...
  function rbenx_match (this: any, _graph: any, node: any, constraintList: any[], synthesize: any, /* constraintToTripleMapping, tripleToConstraintMapping, */ neighborhood: any[], recurse: any, direct: any, semActHandler: any, checkValueExpr: any, trace: any) {
      const rbenx = this;
      let clist: any[] = [], nlist: any[] = []; // list of {state:state number, repeats:stateNo->repetitionCount}

      function resetRepeat (thread: any, repeatedState: number) {
        const trimmedRepeats = Object.keys(thread.repeats).reduce((r: any, k) => {
      ...
      function incrmRepeat (thread: any, repeatedState: number) {
        const incrmedRepeats = Object.keys(thread.repeats).reduce((r: any, k) => {
      ...
      function stateString (state: number, repeats: any): string {
      ...
      function addstate (list: any[], stateNo: number, thread: any, seen?: string[]): number[] {
        seen = seen || [];
        ...
        if (s.c === Split) {
          return s.outs.reduce((ret: number[], o: number) => {
        ...
        // } else if (s.c.type === "OneOf" || s.c.type === "EachOf") { // don't need Rept
          ...
          return [list.push({ // return [new list element index]
            ...
            avail:thread.avail.map((a: any[]) => { // copy parent thread's avail vector
              return a.slice();
            }),
            stack:thread.stack,
        ...
        for (let threadno = 0; threadno < clist.length; ++threadno) {
          ...
          if (taken.length >= min) {
            do {
              ...
              const exprs = rbenx.states.map((x: any) => { return x.c === Rept ? x.expr : null; });
              const newStack = state.stack.map((e: any) => {
              ...
              state.outs.forEach((o: number) => { // single out if NFA includes epsilons
          ...
          if (trace)
            trace[trace.length-1].threads.push({
              ...
              to:nlist.slice(nlistlen).map((x: any) => {
      ...
      if (chosen === null)
        return reportError(localExpect(clist, rbenx.states));
      function reportError (errors: any) { return {
        type: "Failure",
        node: node,
        errors: errors
      } }
      function localExpect (clist: any[], states: any[]) {
    ...
    function matchedToResult (matched: any[], _constraintList: any[], neighborhood: any[], recurse: any, direct: any, semActHandler: any, checkValueExpr: any) {
      let last: any[] = [];
      const errors: any[] = [];
      const skips: any[] = [];
      const ret = matched.reduce((out, m) => {
        let mis = 0;
        let ptr: any = out, t: any;
        ...
        ptr.solutions = m.triples.map((tno: number) => {
          ...
          function diver (focus: any, shape: any, dive: any) {
          ...
          function diveRecurse (focus: any, shapeLabel: any) {
            return diver(focus, shapeLabel, recurse);
          }
          function diveDirect (focus: any, shapeLabel: any) {
