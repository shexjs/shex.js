...
interface TemplateParam {
  name: string;
  kind?: "iri";        // absent = shapeExpr
  extends?: string;    // nominal bound: argument must EXTENDS this label
}
...
export interface ExpandOptions {
  /** distinct instantiations allowed per template (default 100) */
  maxInstances?: number;
}
...
function canonArg (arg: any): string {
...
function mangle (templateId: string, args: any[]): string {
  return templateId + "(" + args.map(a => encodeURIComponent(canonArg(a))).join(",") + ")";
}

function appTrace (templateId: string, args: any[]): string {
  return templateId + "<<" + args.map(canonArg).join(", ") + ">>";
}

function containsParamRef (o: any): boolean {
  if (o === null || typeof o !== "object") return false;
  if (o.type === "ParamRef") return true;
 ...
 */
export function expandTemplates (schema: any, opts: ExpandOptions = {}): any {
  if (!schema)
    return schema;
  ...
  const templates = new Map<string, TemplateDecl>(
    ((schema.templates || []) as TemplateDecl[]).map(t => [t.id, t]));
  const kinds = new Map<string, Map<string, "iri" | "shapeExpr">>(); // per template, inferred+declared param kinds

  const instances = new Map<string, string>();  // canon key -> instance label
  const named = new Map<string, string>();      // canon key -> decl label (pass 0)
  const counts = new Map<string, number>();     // template -> #instantiations
  const emitted: { label: string, decl: any }[] = [];
  const placed = new Set<string>();             // instance labels already in an output slot

  // ── parameter kinds: declared, else inferred from use positions ──────────
  function kindsOf (tpl: TemplateDecl, trace: string[]): Map<string, "iri" | "shapeExpr"> {
    let m = kinds.get(tpl.id);
    if (m) return m;
    m = new Map();
    for (const p of tpl.params) {
      if (p.kind === "iri") m.set(p.name, "iri");
    }
    const seen = new Map<string, "iri" | "shapeExpr">();
    (function scan (o: any, pos: "iri" | "shapeExpr") {
      if (o === null || typeof o !== "object") return;
      if (Array.isArray(o)) { o.forEach(e => scan(e, pos)); return; }
      if (o.type === "ParamRef") {
        const before = seen.get(o.name);
        if (before && before !== pos)
          throw new TemplateExpansionError(
            "parameter ?" + o.name + " of " + tpl.id + " is used both as an IRI and as a shape expression; declare its kind",
            trace);
        seen.set(o.name, pos);
        return;
      }
  ...
  function argAsIri (arg: any, paramName: string, templateId: string, trace: string[]): string {
    ...
    if (arg && arg.type === "NodeConstraint" && typeof arg.datatype === "string"
      ...
      return arg.datatype;
    throw new TemplateExpansionError(
      "parameter ?" + paramName + " of " + templateId + " wants an IRI; got " + canonArg(arg),
      trace);
  }

  // nominal bound: the argument names a declaration that (transitively) EXTENDS the bound
  function checkBound (arg: any, param: TemplateParam, tpl: TemplateDecl, trace: string[]) {
    if (!param.extends) return;
    if (typeof arg !== "string")
      throw new TemplateExpansionError(
        "parameter ?" + param.name + " of " + tpl.id + " is bounded by EXTENDS <" + param.extends + ">, so its argument must be a shape reference; got " + canonArg(arg),
        trace);
    const seen = new Set<string>();
    const walk = (label: string): boolean => {
      if (label === param.extends) return true;
      if (seen.has(label)) return false;
      seen.add(label);
      const decl = (schema.shapes || []).find((d: any) => d.id === label);
      const ext = decl && decl.shapeExpr && decl.shapeExpr.extends || [];
      return ext.some((e: any) => typeof e === "string" && walk(e));
    };
    if (!walk(arg))
      throw new TemplateExpansionError(
        "argument @" + arg + " for ?" + param.name + " of " + tpl.id + " does not EXTENDS <" + param.extends + ">",
        trace);
  }

  type Env = Map<string, { param: TemplateParam, kind: "iri" | "shapeExpr", value: any }> | null;

  function instantiate (templateId: string, rawArgs: any[], env: Env, trace: string[]): string {
    ...
    if (args.length !== tpl.params.length)
      throw new TemplateExpansionError(
        tpl.id + " takes " + tpl.params.length + " parameter(s) (" + tpl.params.map(p => "?" + p.name).join(", ") + "); got " + args.length,
        trace);
    ...
    if (count > maxInstances)
      throw new TemplateExpansionError(
        templateId + " exceeded " + maxInstances + " instantiations — does its recursion grow its arguments?",
        here);
  ...
  function rewrite (o: any, env: Env, trace: string[], pos: "iri" | "shapeExpr"): any {
    if (o === null || typeof o !== "object")
      return o;
    if (Array.isArray(o))
    ...
    if (o.type === "ParamRef") {
      ...
      if (bound.kind === "iri")
        throw new TemplateExpansionError(
          "IRI parameter ?" + o.name + " used as a shape expression", trace);
  ...
  function canonKeyOf (app: any): string {
  ...
  for (const [k, v] of Object.entries(schema)) {
    if (k === "templates" || k[0] === "_") continue; // _index & friends describe the unexpanded schema
    if (k === "shapes") out.shapes = outShapes;
    else if (k === "start") out.start = rewrite(v, null, [], "shapeExpr");
    else out[k] = v;
  }
