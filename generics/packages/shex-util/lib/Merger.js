...
exports.StoreDuplicates = exports.Merger = void 0;
exports.yyllocToString = yyllocToString;
...
class Merger {
    ...
    mergeMap(attr, myOverwrite = this.overwrite) {
        (this.left[attr] || new Map()).forEach((_value, key) => {
            if (!(attr in this.ret))
                this.ret[attr] = new Map();
            this.ret[attr].set(key, this.left[attr].get(key));
        });
        (this.right[attr] || new Map()).forEach((_value, key) => {
    ...
    merge(...args) {
        ...
        if ("imports" in this.right)
            if (!("imports" in this.left)) {
                ...
                [].push.apply(this.ret.imports, this.right.imports.filter((mprt) => this.ret.imports.indexOf(mprt) === -1));
            }
        // startActs
        ...
        const lindex = this.left._index || visitor_1.ShExIndexVisitor.index(this.left);
        // templates (strawman: doc/templates.md) — merged like shapes, keyed by id
        const ltemplates = new Map((this.left.templates || []).map((t) => [t.id, t]));
        if (!this.inPlace)
            (this.left.templates || []).forEach((lt) => {
                if (!("templates" in this.ret))
                    this.ret.templates = [];
                this.ret.templates.push(lt);
            });
        (this.right.templates || []).forEach((rt) => {
            if (!("templates" in this.ret))
                this.ret.templates = [];
        ...
        if (this.left._index || this.right._index)
            this.ret._index = visitor_1.ShExIndexVisitor.index(this.ret); // inefficient; could build above
        return this.ret;
    }
    /**
    ...
    static warnDuplicates(type, left, right, leftLloc, rightLloc, _leftMeta, _rightMeta) {
        ...
        const wheresStr = [];
        if (leftLloc)
            wheresStr.push(yyllocToString(leftLloc));
        if (rightLloc)
            wheresStr.push(yyllocToString(rightLloc));
        if (lStr === rStr) {
...
class StoreDuplicates {
    ...
    overwrite(type, left, _right, leftLloc, rightLloc, leftMeta, rightMeta) {
        if (type === "_prefixes")
            return false;
        if (type !== "shapeDecl")
        ...
        if (!this.duplicates[id])
            this.duplicates[id] = [Object.assign(Object.assign({}, leftLloc), { importers: leftMeta.importers })];
        this.duplicates[id].push(Object.assign(Object.assign({}, rightLloc), { importers: rightMeta.importers }));
        return false; // keep left/old assignment
    }
}
exports.StoreDuplicates = StoreDuplicates;
