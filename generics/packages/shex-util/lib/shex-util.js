...
const ShExHumanErrorWriter = require("./shex-human-error-writer");
const SX = { _namespace: "http://www.w3.org/ns/shex#" };
...
const RDF = { _namespace: "http://www.w3.org/1999/02/22-rdf-syntax-ns#" };
["type", "first", "rest", "nil"].forEach(p => {
    RDF[p] = RDF._namespace + p;
});
const OWL = { _namespace: "http://www.w3.org/2002/07/owl#" };
["Thing"].forEach(p => {
...
class SchemaStructureValidator extends visitor_1.ShExVisitor {
    constructor(schema, options, negativeDeps, positiveDeps) {
        ...
        this.inTE = false;
        this.index = schema.index || visitor_1.ShExIndexVisitor.index(schema);
    }
...
const ShExUtil = {
    ...
    expandTemplates: function (schema, opts) {
    ...
    ShExJtoAS: function (schema) {
        ...
        schema._index = visitor_1.ShExIndexVisitor.index(schema);
        return schema;
    },
    ...
    ShExRtoShExJ: function (schema) {
        ...
        class ShExRVisitor extends visitor_1.ShExVisitor {
    ...
    BiDiClosure: function () {
        return {
            ...
            inCycle: [],
            test: function () {
                function expect(l, r) { const ls = JSON.stringify(l), rs = JSON.stringify(r); if (ls !== rs)
                    throw Error(ls + " !== " + rs); }
                ...
                this.add(5, 6);
                expect(this.needs, { 5: [6, 7], 6: [7], 1: [2, 3, 4], 2: [3, 4], 3: [4] });
                expect(this.neededBy, { 7: [6, 5], 6: [5], 3: [2, 1], 2: [1], 4: [3, 2, 1] });
                this.add(5, 7);
                expect(this.needs, { 5: [6, 7], 6: [7], 1: [2, 3, 4], 2: [3, 4], 3: [4] });
                expect(this.neededBy, { 7: [6, 5], 6: [5], 3: [2, 1], 2: [1], 4: [3, 2, 1] });
                this.add(7, 8);
                expect(this.needs, { 5: [6, 7, 8], 6: [7, 8], 7: [8], 1: [2, 3, 4], 2: [3, 4], 3: [4] });
                expect(this.neededBy, { 7: [6, 5], 6: [5], 8: [7, 6, 5], 3: [2, 1], 2: [1], 4: [3, 2, 1] });
                this.add(4, 5);
            ...
            add: function (needer, needie, _negated) {
                const r = this;
    ...
    nestShapes: function (schema, options = {}) {
        ...
        let nestables = Object.keys(shapeReferences).filter(label => shapeReferences[label].length === 1
        ...
        ).filter(nestable => !('noNestPattern' in options)
            || !nestable.match(RegExp(options.noNestPattern))).reduce((acc, label) => {
            acc[label] = {
    ...
    getPredicateUsage: function (schema, untyped = {}) {
        ...
        Object.keys(schema.shapes).forEach(label => {
            ...
            if (shapeExpr.type === 'Shape') {
                (shapeExpr.extends || []).forEach((superShape) => shapeHierarchy.add(superShape.reference, label));
            }
        });
        Object.keys(schema.shapes).forEach(label => {
                        ...
                        if (typeof curType === 'object' || curType === null) {
                            ...
                            if (idx === -1) {
                                let intersection = shapeHierarchy.parents[curType]
                                    ? shapeHierarchy.parents[curType].filter((lab) => -1 !== shapeHierarchy.parents[newType].indexOf(lab))
                                    : [];
    ...
    simpleTripleConstraints: function (shape) {
        ...
        if (shape.expression.type === 'EachOf' &&
            !(shape.expression.expressions.find((expr) => expr.type !== 'TripleConstraint'))) {
    ...
    partition: function (schema, includes, deps, cantFind) {
        const inputIndex = schema._index || visitor_1.ShExIndexVisitor.index(schema);
     ...
     * @schema: input schema
     */
    flatten: function (schema, _deps, _cantFind) {
        const v = new visitor_1.ShExVisitor();
    ...
    validateSchema: function (schema, options) {
        ...
        return jsonObject.results.bindings.map((row) => {
            ...
            return selects.map((sel) => {
                ...
                switch (elt.type) {
                    ...
                    case "literal":
                        return dataFactory.literal(elt.value, "xml:lang" in elt
                            ? elt["xml:lang"]
                            : "datatype" in elt
 ...
 */
const exported = AddShExUtil(AddShExUtil);
module.exports = exported;
//# sourceMappingURL=shex-util.js.map