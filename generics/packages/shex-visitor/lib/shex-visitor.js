...
exports.ShExIndexVisitor = exports.ShExVisitor = void 0;
class ShExVisitor {
    ...
    visitSchema(schema, ...args) {
        ...
        this._maybeSet(schema, ret, "Schema", ["@context", "prefixes", "base", "imports", "startActs", "start", "templates", "shapes"], ["_base", "_prefixes", "_index", "_sourceMap", "_locations", "_exprLocations"], ...args);
        return ret;
    }
    // ── templates (strawman: doc/templates.md) ──
    visitTemplates(templates, ...args) {
        return templates.map(t => this.visitTemplateDecl(t, ...args));
    }
    visitTemplateDecl(decl, ...args) {
        return this._maybeSet(decl, { type: "TemplateDecl" }, "TemplateDecl", ["id", "params", "shapeExpr"], null, ...args);
    }
    visitParams(params, ..._args) {
        return params.map(p => Object.assign({}, p));
    }
    visitTemplateApp(expr, ...args) {
        return this._maybeSet(expr, { type: "TemplateApp" }, "TemplateApp", ["template", "args"], null, ...args);
    }
    visitTemplate(template, ..._args) {
        return template;
    }
    visitArgs(appArgs, ...args) {
        return appArgs.map(a => this.visitShapeExpr(a, ...args));
    }
    visitParamRef(expr, ...args) {
        return this._maybeSet(expr, { type: "ParamRef" }, "ParamRef", ["name"], null, ...args);
    }
    visitPrefixes(prefixes, ..._args) {
    ...
    visitSemAct(semAct, _label, ...args) {
    ...
    visitShapes(shapes, ...args) {
        if (shapes === undefined)
            return undefined;
        return shapes.map(shapeExpr => this.visitShapeDecl(shapeExpr, ...args));
    }
    ...
    visitShapeExpr(expr, ...args) {
        ...
        if (expr.type === "TemplateApp") // strawman: doc/templates.md
    ...
    visitInclusion(inclusion, ..._args) {
