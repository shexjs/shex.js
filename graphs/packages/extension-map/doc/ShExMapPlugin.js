 ...
 * this runs on, which the descriptor's `scripts` names and the app fetches.
 *
 * This is doc/ShExMapPlugin.js's source (tsconfig.plugin.json compiles
 * src/plugin/ into doc/); edit here and run `npm run build`.
 */
...
class DirectShExMaterializer {
    ...
    async invoke(fixedMap, validationTracker, time, _done, _currentAction) {
        ...
        this.shapeMap.forEach(pair => {
            ...
            quads.forEach((quad, i) => this.provenance.push(Object.assign({ quad }, (materializer.provenance || [])[i])));
        });
...
class RemoteShExMaterializer {
    ...
    update(msg) {
        ...
        const quads = msg.data.quads.map(q => WorkerMarshalling.jsonTripleToRdfjsTriple(q, RdfJs.DataFactory));
...
const ShExMapVerbs = {
    ...
    getMaterializer(schema, shapeMap, resultBindings, staticVars) {
        return this.remote
            ? new RemoteShExMaterializer(schema, shapeMap, resultBindings, staticVars, this.resultsWidget, () => this.materialize(), WorkerUrl, this.MapModule)
    ...
    async materializeAsync() {
        ...
        this.resultsWidget.start();
        try {
            const { outputSchema, resultBindings, staticVars, outputShapeMap } = await this.collectMaterializationInputs();
    ...
    showMaterialization() {
        ...
        if (tabs.find('[href="#' + MATERIALIZATION_TAB + '"]').length === 0)
            tabs.find("> ul").append($("<li/>").append($("<a/>", { href: "#materializationResults" }).text("materialization")));
    ...
    bindingStateText(thread) {
        ...
        return "binding tree (✓ = consumed by this thread; → = cursor):\n" +
            frames.map((frame, i) => (i === thread.frame ? "→ " : "  ") + "frame " + i + ":  " +
                Object.keys(frame).map(v => pname(v) + (usedSet.has(i + " " + v) ? " ✓" : "")).join("  ")).join("\n");
    },
    ...
    annotateBindingState(thread) {
        ...
        byPath.forEach(({ path, variable, consumed, frames }) => {
            ...
            this.bindingRangesAt(text, path).forEach(range => marks.push({ from: range.from, to: range.to, cls, title }));
        });
        pane.annotate(marks);
    },
    ...
    previewThread(thread, complete, label) {
        ...
        if (complete && session) {
            ...
            this.reportMaterialization(thread.provenance, thread.quads[thread.quads.length - 1] || null);
    ...
    writeNestedTurtle(quads, onDone) {
        ...
        try {
            ...
            // orderedForNesting
            writer.addQuads(ordered.filter(q => store.countQuads(q.subject, q.predicate, q.object, q.graph) > 0));
            writer.end(onDone);
        }
        catch (e) {
    ...
    renderAcceptAlternatives(materializer, outputShapeMap) {
        ...
        if (accepts.length < 2)
            return;
        const div = $("<div/>", { class: "dbgAlternatives" }).append(accepts.length + " viable materializations (showing the most-consuming): ");
        accepts.forEach((a, i) => {
    ...
    endDebugSession(stopped) {
        ...
        if (session.dbg.error) {
            ...
            this.renderMaterializedGraph(generatedGraph, session.outputShapeMap, session.materializer.provenance);
    ...
    anchorMaterializationFailures(e, lastReport) {
        ...
        try {
            ...
            const located = ShExWebApp.EditorServices.locateInParsed(this.Caches.outputSchema.selection.val(), this.Caches.outputSchema.parsed);
            ...
            pane.setDiagnostics([]
                .concat((e && Array.isArray(e.failures) ? e.failures : []).map(failure => anchored(failure, "error", failure.variable ? "no binding for <" + failure.variable + ">"
                : (failure.error || failure.code || "materialization failure"))))
                .concat((report.unboundVariables || []).map(failure => anchored(failure, "warning", "<" + failure.variable + "> is bound nowhere (bindings or statics); branches needing it were abandoned")))
    ...
    reportMaterialization(provenance, focus) {
        ...
        try {
            ...
            const rendered = this.materializationPanes.map(({ pane, text }) => ({
                pane,
                text,
                pairs: ShExWebApp.EditorServices.mapMaterialization(provenance, located, ShExWebApp.EditorServices.parseTurtle(text, { baseIRI: this.Caches.outputSchema.meta.base })),
            }));
    ...
    setMaterializationHovers(rendered, focus) {
        ...
        const wipe = () => {
            ...
            if (bindingsPane)
                bindingsPane.clearHighlights();
            if (staticsPane)
                staticsPane.clearHighlights();
        ...
        const termRanges = (p) => [].concat(p.anchors.objectParts || (p.anchors.object ? [p.anchors.object] : []), p.anchors.subjectParts || (p.anchors.subject ? [p.anchors.subject] : []), p.anchors.predicate ? [p.anchors.predicate] : []);
        ...
        const show = (group, hoveredPane, pinning) => {
            ...
            schemaPane.highlight(group.flatMap(p => p.schemaParts || (p.schema ? [p.schema] : [])), cls, { scroll: hoveredPane !== schemaPane });
            ...
            if (bindingsPane)
                bindingsPane.highlight(group.filter(p => !p.statics).flatMap(p => p.variables.flatMap(v => this.bindingRanges(bindingsText, v, p.frame))), cls, { scroll: hoveredPane !== bindingsPane });
            if (staticsPane)
                staticsPane.highlight(group.filter(p => p.statics).flatMap(p => p.variables.flatMap(v => this.variableRanges(staticsText, v, null))), cls, { scroll: hoveredPane !== staticsPane });
        ...
        rendered.forEach(({ pane, pairs }) => {
            pane.setHoverRegions(pairs.flatMap(p => termRanges(p).map(r => ({ from: r.from, to: r.to,
                enter: () => show([p], pane),
                click: freeze([p], pane) }))), clearAll);
        });
        ...
        schemaPane.setHoverRegions([...bySchema.values()].flatMap(group => (group[0].schemaParts || [group[0].schema]).map(r => ({ from: r.from, to: r.to,
            enter: () => show(group, schemaPane),
            click: freeze(group, schemaPane) }))), clearAll);
                ...
                if (origins) {
                    ...
                    if (byPath.size > 0) {
                        byPath.forEach(({ path, frames }) => {
                            ...
                            this.bindingRangesAt(text, path).forEach(r => regions.push({ from: r.from, to: r.to,
    ...
    addResult(error, result) {
        const div = $("<div/>", { class: "passes" }).append($("<span/>", { class: "shapeMap" }).append("# ", $("<span/>", { class: "data" }).text($("#outputShapeMap").val())));
