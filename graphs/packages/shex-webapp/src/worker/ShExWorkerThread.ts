 ...
 */
/** what a plugin's worker half registers */
interface WorkerPlugin {
  ready?: Promise<unknown>;
  register?: (validator: any, api: any) => void;
  requests?: {[request: string]: (msg: MessageEvent, api: any) => unknown};
  neighborhoods?: any[];
}

const WorkerPlugins: WorkerPlugin[] = [];
const importedPlugins = new Set();

function registerWorkerPlugin (plugin: WorkerPlugin) {
...
function importPlugins (urls: string[] | undefined) {
  (urls || []).forEach(url => {
...
self.onmessage = async function (msg: MessageEvent) {
...
try {
  ...
  case "create":
    ...
    const inputData = msg.data.neighborhood
          ...
          : ShExWebApp.RdfJsDb(makeStaticDB(msg.data.data.map((t: any) => WorkerMarshalling.jsonTripleToRdfjsTriple(t, N3js.DataFactory))));
  ...
  case "validate":
    ...
    for (let currentEntry = 0; currentEntry < queryMap.length; ) {
      ...
      time = Date.now() - time;
      newResults.forEach(function (res: any) {
      });
      // Merge into results.
  ...
  case "debugValidate": {
    ...
    const dbgDb = ShExWebApp.RdfJsDb(makeStaticDB(
      (msg.data.data || []).map((t: any) => WorkerMarshalling.jsonTripleToRdfjsTriple(t, N3js.DataFactory))));
    ...
    const gate = new ShExWebApp.WorkerGate(
      msg.data.sab,
      (m: any) => self.postMessage({ response: "paused", event: m.event }),
    ...
    const dbgTracker = ShExWebApp.eventTracker((event: any) => {
      if (event.type !== "known")
        gate.gate(event);
    });
    dbgOptions.debugHooks = {
      onConstraint: (tc: any, ctx: any) => gate.gate({
    ...
    const dbgMap = (msg.data.queryMap || []).map((m: any) =>
    ...
    try {
      ...
      self.postMessage({ response: "done", conformant: dbgResults.every((r: any) => r && r.status === "conformant") });
    } catch (e: any) {
      if (e && e.isDebugAbort)
...
function makeStaticDB (quads: any[]) {
  ...
  function makeRelayTracker () {
    const logger = {
      recurse: (x: any) => { self.postMessage({ response: "recurse", x: x }); return x; },
      known: (x: any) => { self.postMessage({ response: "known", x: x }); return x; },
      enter: (point: any, label: any) => { self.postMessage({ response: "enter", point: point, label: label }); },
      exit: (point: any, label: any, ret: any) => { self.postMessage({ response: "exit", point: point, label: label, ret: null }); }, /* don't ship big ret structures */
    };
    return logger;
  }
...
function postSlurpedPages () {
  if (!slurping || inputDb === null || typeof inputDb.loadedPages !== "function")
    return;
  const fresh = inputDb.loadedPages().filter((page: any) => !postedPages.has(page.id));
  if (fresh.length === 0)
    return;
  fresh.forEach((page: any) => postedPages.add(page.id));
  self.postMessage({ response: "slurpedPages", pages: fresh });
}

/** the neighborhood module the app named, by the id both ends know it by */
function neighborhoodModule (id: string) {
  ...
  const found = modules.find((m: any) => moduleId(m) === id);
  if (found === undefined)
...
function asyncFace (module: any, params: any, tracker: any) {
...
function queryTracker () {
  return {
    start: function (isOut: boolean, term: any, shapeLabel: any) {
    ...
    end: function (quads: any[], time: number, token: number) {
      self.postMessage({ response: "finishQuery", token: token, quads: quads.map((t: any) => WorkerMarshalling.rdfjsTripleToJsonTriple(t)), time: time });
    },
    fail: function (error: any, time: number, token: number) {
