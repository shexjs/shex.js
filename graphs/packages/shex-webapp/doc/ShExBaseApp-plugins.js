...
mixin(ShExBaseApp, {
        ...
        if (typeof ext.init === "function" && !ext.initialized) {
            ...
            try {
                ...
                if (readying && typeof readying.then === "function")
                    return readying.catch((e) => this.resultsWidget.failMessage(e, "loading " + (ext.label || ext.id)));
            }
            catch (e) {
    ...
    unloadPlugin(id) {
        ...
        (ext.neighborhoods || []).forEach(module => this.neighborhoods.remove(ShExWebApp.NeighborhoodApi.moduleId(module)));
    ...
    removeResultsTab(id) {
        ...
        if (tabs.length === 0)
            return;
        tabs.find("> ul > li").filter((i, li) => $(li).children("a").attr("href") === "#" + id).remove();
        $("#" + id).remove();
    ...
    resultsTabFor(id, label, { empty = false } = {}) {
        ...
        if (tabs.length === 0) {
            ...
            $("<ul/>").append($("<li/>").append($("<a/>", { href: "#" + APP_RESULTS_TAB }).text("validation"))).appendTo(tabs);
    ...
    showScreen(id) {
        if ($("#screen").length === 0)
            return;
        $("#screen").val(id);
        $("#screenTabs button").each((i, b) => $(b).attr("aria-selected", String($(b).attr("data-screen") === id)));
    ...
    makeRenderer() {
        const cls = pluginDescriptors().reduce((sofar, ext) => typeof ext.results === "function" ? ext.results(sofar) : sofar, ShExResultsRenderer);
